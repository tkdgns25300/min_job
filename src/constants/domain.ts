 ...
 * ⛔ **`ANY`("무관")를 뺐다**(2026-08-27). DATA §2가 **`NULL`을 이미 "무관"으로 정의**하는데
 *    `ANY`가 그 두 번째 철자였다 — 같은 뜻을 적는 방법이 둘이면 아무도 둘 다 쓰지 않는다.
 *    실제로 `ANY` 행은 **0건**이었고, `filter-jobs.ts`가 값 없는 공고를 버리므로 **구직자가
 *    "무관" 칩을 누르면 887건 중 0건**이 나왔다(무관에 해당하는 `NULL` 263건이 통째로 빠진 채).
 *    ⚠️ **반대로 고치지 않은 이유** — `ANY`를 `NULL`에도 걸리게 하면 "원문에 안 적혀 있을 뿐"인
 *       공고를 "자격을 안 따진다"로 **단정**하게 된다. `housingLabel`이 `null`(정보 없음)과
 *       `false`(명시적 미제공)를 가르는 것과 같은 이유다 — 공고가 하지 않은 말을 우리가 하게 된다.
 *    칩을 없애도 잃는 것이 없다: 자격을 안 따지는 구직자는 **필터를 걸지 않으면** 전부 본다.
 *    ⚠️ DB CHECK에는 `'ANY'`가 남아 있다(마이그레이션 안 함 · 0건이라 급하지 않다). 크롤러가
 *       보내더라도 `keyOf`가 `null`로 좁히는데, `ANY ≡ NULL`이라 **뜻이 보존된다**.
 */
