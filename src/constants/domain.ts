...
/**
 * 비슷한 공고 6장 중 **위에서 몇 장이 광고 칸인가**(운영자 결정 2026-09-05).
 ...
 * 노출을 묽게 하는 결정이다(SPEC 수익화 절 "정원이 아니라 값을 올린다") — 늘리는 것은 운영자 판단이다.
 */
...
//   CANCELLED — **게재 시작 전** 취소. 전액 환불(시작 전이라 일할 계산이 없다). 결제 완료 액션이
//     자리를 줄 수 없을 때(주문 불일치·남의 공고·정원 마감·경합) 스스로 적는 값이기도 하다.
//   REFUNDED  — 게재 시작 뒤 환불한 것. **남은 일수만큼 일할 환불**(요청 접수일 기준 · 약관 제10조 ② · 2026-09-04
//               운영자 결정) — PortOne 콘솔에서 부분 취소한 뒤 원장 상태를 적는다. 환불 도구는 만들지 않는다.
