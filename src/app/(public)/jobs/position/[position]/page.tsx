...
// position 랜딩 — Next가 **정적으로 찾아야 하는 것**만 여기 두고, 판단은 `../../facet-page`가 한다.
// 세 라우트가 축 이름만 다르다.

const AXIS = "position" as const;

type Params = { params: Promise<{ position: string }> };

/** 값이 **닫힌 집합**이라 빌드타임에 다 만든다(공고 상세와 달리 나중에 늘어나지 않는다) */
export function generateStaticParams() {
  return facetKeys(AXIS).map((key) => ({ position: facetSlug(key) }));
}

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  return facetMetadata(AXIS, (await params).position);
}
...
async function FacetSlug({ params }: Params) {
  return <FacetContent axis={AXIS} slug={(await params).position} />;
}
