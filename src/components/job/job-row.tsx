...
// (`splitListAds`). 유료 노출은 organic처럼 보이되 회색 텍스트 "광고" 한 단어로만 표시한다 — 등급 차이는
// 자리(스페셜 → 플러스 순)로만, 배경 틴트·등급명·테두리 칩 없음(SPEC 수익화 절 · 표시광고·신뢰).
// 전체 행 클릭 = 상세로(stretched Link), 책갈피만 별도 클릭(z-10).
export function JobRow({ job, ad = false }: { job: JobCard; ad?: boolean }) {
