...
/**
 * 사례비 **카드용** — **금액이면 금액, 아니면 "협의"** 둘뿐이다. 상세 페이지는 `formatPay`(원문 그대로).
 *
 * 공개 공고의 61%가 금액 없이 자유 텍스트만 갖고(2026-08-29 실측 953건 중 579) 표기가 330가지다
 * ("교회 내규에 따름"·"면담 후 결정"·"12개월+2개월(상여금),재학생 장학금…"). 카드에 그대로 두면 칸이
 * 무너지고(174자), 잘라 두면 더럽고, "교회 내규"/"협의" 두 라벨로 접어도 훑어 읽기에 방해였다
 * (운영자 판단 2026-08-30). 카드는 **금액을 비교하는 자리**라 금액 아닌 것은 전부 "협의"로 접고,
 * 무엇을 협의하는지는 상세에서 원문으로 읽는다.
 * ⚠️ 표시 규칙이지 데이터가 아니다 — DB·상세·검수 화면은 원문을 그대로 쓴다.
 */
export function formatPayShort(job: Pick<JobCard, "payMin" | "payMax" | "payPeriod">): string {
  if (job.payMin !== null || job.payMax !== null) return formatPay({ ...job, payNote: null });
  return "협의";
}
