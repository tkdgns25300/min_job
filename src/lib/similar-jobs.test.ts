...
describe("문(자격) — 통과 못 하면 가까워도 안 보인다", () => {
  it("사역직 공고에 일반직 후보는 어떤 단계에서도 섞이지 않는다", () => {
    ...
    const pick = pickSimilarJobs(base, [general], TODAY, 6);
  ...
  it("직분이 하나도 안 겹치면 엄격 단계에서 탈락한다 — 겹치는 후보가 먼저 선다", () => {
    ...
    const pick = pickSimilarJobs(base, [pastor, evangelist], TODAY, 6);
  ...
  it("직분은 양쪽 다 있을 때만 대조한다 — 혼합 공고 페이지에 일반직 공고가 문을 넘는다", () => {
    ...
    const pick = pickSimilarJobs(mixedBase, [generalOnly], TODAY, 6);
  ...
  it("교단은 둘 다 밝혀졌는데 다를 때만 탈락 — 미상은 통과한다", () => {
    ...
    const pick = pickSimilarJobs(base, [other, unknown], TODAY, 1);
    expect(ids(pick.organic)).toEqual([unknown.id]);
  });

  it("같은 교회의 다른 공고는 뺀다", () => {
    const sameChurch = candidate({ churchName: "기준교회" });
    const pick = pickSimilarJobs(base, [sameChurch], TODAY, 6);
    expect(pick.organic).toEqual([]);
  });
});
...
describe("점수(선호) — 같은 지역 3 · 교단 2 · 부서 2 · 고용형태 1, 동점은 최신순", () => {
  it("지역이 같은 것이 부서·고용형태가 같은 것보다 앞선다", () => {
    ...
    const pick = pickSimilarJobs(base, [deptAndType, regionOnly, regionAndDenom], TODAY, 6);
  ...
  it("동점이면 최신 공고가 먼저다", () => {
    ...
    const pick = pickSimilarJobs(base, [older, newer], TODAY, 6);
  ...
  it("미상끼리는 '같다'로 치지 않는다", () => {
    ...
    const pick = pickSimilarJobs(unknownBase, [alsoUnknown, known], TODAY, 6);
...
describe("첫 칸 광고 — 문 통과 + 같은 지역 + 유료 노출 중", () => {
  ...
  it("다른 지역 유료 공고는 광고가 아니다 — 유기 후보로만 남는다", () => {
    ...
    const pick = pickSimilarJobs(base, [farAd], TODAY, 6);
    expect(pick.ad).toBeNull();
    expect(ids(pick.organic)).toEqual([farAd.id]);
  });

  it("기한이 지난 유료 공고는 광고가 아니다", () => {
    const expired = paid({ featuredUntil: "2026-09-02" });
    expect(pickSimilarJobs(base, [expired], TODAY, 6).ad).toBeNull();
  });

  it("문을 못 넘는 유료 공고(직분 불일치)는 광고가 아니다", () => {
    const pastorAd = paid({ position: ["ASSOCIATE_PASTOR"] });
    expect(pickSimilarJobs(base, [pastorAd], TODAY, 6).ad).toBeNull();
  });

  it("지역 미상 기준 공고에는 광고가 서지 않는다", () => {
    const unknownRegion = candidate({ id: "b3", region: null });
    expect(pickSimilarJobs(unknownRegion, [paid()], TODAY, 6).ad).toBeNull();
  });

  it("여럿이면 기준 공고 id로 결정적으로 하나를 고른다 — 같은 입력이면 늘 같은 광고", () => {
    const ads = [paid(), paid(), paid()];
    const first = pickSimilarJobs(base, ads, TODAY, 6).ad?.id;
    const again = pickSimilarJobs(base, [...ads].reverse(), TODAY, 6).ad?.id;
    expect(first).toBe(again);
    // 다른 기준 공고들은 서로 다른 광고를 고를 수 있다(페이지마다 나눠 보이는 근거) — 셋 중 둘 이상이 나온다
    const chosen = new Set(
      ["p1", "p2", "p3", "p4", "p5", "p6"].map(
        (id) => pickSimilarJobs(candidate({ id }), ads, TODAY, 6).ad?.id,
      ),
    );
