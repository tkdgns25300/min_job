...
>
> ▶ **2026-09-05 파비콘·앱 아이콘**: 프로젝트를 만든 날의 **Next 기본 파비콘이 그대로 있었다**. 브랜드 마크로 교체 — 딥그린 타일(`--brand-900`) + 골드 M(`--gold`), 로고가 "**Min**Job"이라 16px에 들어갈 글자가 하나뿐이다. `src/app/`의 **icon.svg**(벡터·주 아이콘) · **favicon.ico**(16·32·48을 한 파일에 · `/favicon.ico`를 그냥 찍는 크롤러용) · **apple-icon.png**(180 · iOS가 모서리를 깎으므로 꽉 찬 사각·불투명). Next 파일 규약이라 link 태그는 자동 주입된다. 시안 넷을 16~120px로 나란히 뽑아 골랐고, 고른 기준은 **가장 작은 크기**다 — 여백이 넉넉한 안은 120px에서 차분했지만 16px에서 획이 붙었다. 만드는 법은 `scripts/make-icons.py`(표준 라이브러리만 — 로컬에 SVG 래스터라이저가 없어 폴리곤을 직접 찍고 PNG·ICO를 엮는다). ⚠️ **M을 폰트로 그리지 않는다** — 파비콘은 만드는 기계의 폰트나 보는 브라우저의 폰트에 따라 달라지면 안 되므로 좌표로 박아 뒀다.
>
> ▶ **2026-09-05 소개에 "왜 만들었나요" 신설**(운영자 요청): 1인 신규 서비스라 기능 설명보다 **누가 왜 만들었나**가 신뢰를 만든다고 보고, 개인적인 동기를 세 문단으로 밝혔다 — ① 아버지·누나가 사역 중이고 그 청빙 과정을 가까이서 봤다 ② 만든 사람은 **예장합동 교단 교회 출석** ③ **이단·사이비 공고는 단호히 거절**(수집 단계에서 걸러내고, 교회 직접 등록은 증빙으로 실체를 확인하며, 놓친 건은 이메일 신고로 받아 내린다 — 셋 다 실제로 하는 일이다). **이 페이지에서 유일한 1인칭("저") 구획**이고, 그것이 이 절의 요점이라 다른 절로 번지게 하지 않는다(나머지는 "저희"). 함께 다듬은 카피: 히어로 태그라인은 **"청빙 공고를 한 곳에서 확인하세요"**(흩어져 있다는 말은 키커가 이미 한다), "무엇이 좋은가요" 두 줄은 **어미를 하나로**("~할 수 있습니다"), FAQ 답은 **"수정하겠습니다"**(원문 대조 뒤에 고치므로 속도는 약속하지 않는다). 정본 = SPEC 소개 절.
>
> ▶ **2026-09-05 스페셜 값 인하**(운영자 결정): 최상단을 **1주 6.9 / 2주 12.9 / 4주 20.9만**으로(할인 폭·끝자리는 다른 등급과 같은 관례). 트래픽 실적이 없는 동안은 벤치마크와 같은 급의 값을 부르지 않는다 — 값이 첫 판매를 막는 쪽이 더 손해다. 사다리 간격이 좁아진 만큼 상위 등급의 설득은 **자리 수**(홈 3칸·목록 5줄)가 맡는다. 가격은 `EXPOSURE_PRODUCTS.prices` 단일 소스라 코드는 세 줄이고 요금 페이지·결제 화면·서버 금액 검증이 함께 따라온다. 함께: `/pricing` 히어로 제목 **"공고를 더 많은 사역자에게 알리세요"**("노출"은 우리 쪽 용어다), 홈 히어로는 **"지역·부서·사례비"** 순(찾는 순서대로).
>
...
> ▶ **2026-09-02 BM·노출 상품 재기획 확정(§7 순서 3)**: 대형사·IT/구독형·성과형·기독 니치 네 갈래를 다시 조사하고(§9 E) 우리 실데이터에 대 봤다 — 지역×직분 91조합 중 71개가 20건 이하라 **"상단 고정"은 네 조합에서만 실체**, 측정 인프라 0이라 **성과형은 후보가 아님**, 대표광고 "구좌 한정"은 카피만 있고 수가 없었다. 확정: **자리 셋**(홈 카드 3칸 · 목록 상단 로우 최대 5줄 · 연관 첫 칸) + **사다리 3등급**(스페셜·정원 3 → 플러스·정원 2 → 기본·정원 없음 · 값은 SPEC 상품표) · 광고는 그 화면의 결과에 원래 들어갈 공고일 때만 맨 위 · 정렬 1차 키 폐기 · 런칭 프로모션 없음 · **비슷한 공고 규칙 교체**(자격 문 → 선호 점수 → 보충 — 실측 전도사 페이지 6장 중 부목사 4장이 오던 것을 잡았다). 정본 = SPEC 수익화·공고 상세 절. **자리·라벨·상품표·비슷한 공고 규칙은 2026-09-03 구현**, 결제 적용·정원 판정은 1-8.
>
...
3. ~~BM·노출 상품 재기획~~ ✅ **확정(2026-09-02)** — 자리 셋 + 사다리 3등급(스페셜·플러스·기본) · 정원 3·2 · 값은 SPEC 상품표 · 비슷한 공고 규칙 교체 · 런칭 프로모션 없음. 정본 = SPEC 수익화 절, 근거 = §9. 자리·상품표는 2026-09-03 구현, 결제 적용은 4에서
...
### C. 민잡 상품표 (확정 2026-09-02 · 개정 2026-09-05)
...
VAT 포함 · 2주 −5% · 4주 −25%. 등급 간 비율은 갓피플 4:2:1을 참고한 사다리다(값 자체는 우리 실적에 맞춘다). 자리별 노출 조건·비슷한 공고 규칙 = SPEC.
...
- **정가로 시작, 런칭 프로모션 없음**(운영자 결정 2026-09-02). 값은 벤치마크와 같은 급으로 올리지 않는다 — 트래픽 0인 새 사이트가 그 값을 부를 근거가 없다(최상단 6.9만). 대신 한 번 정한 값을 할인으로 흔들지 않는다.
...
갓피플(3종·8칸·카테고리 목록)보다 **낮은 값에, 자리는 더 희소하게(3·2), 노출은 더 정확하게(자격·지역이 맞는 곳에만)**. 대형사의 매트릭스도 성과형도 1인 운영엔 부적합 — 사다리 3단이 상한이다.

### G. `/pricing` 페이지 구현 (2026-07-09 — ✅ 2026-09-03 3등급으로 전환)
히어로 → **상품 4카드**(무료·기본 2.9만·플러스 4.9만·스페셜 6.9만/주 — 문구·비교표는 `EXPOSURE_PRODUCTS`에서 파생, 가격 공개 + `노출 신청하기` → `/mypage/church/promote`) → **한눈에 비교** → **믿고 노출하세요**(getCoverageStats 실집계 + 하단 슬림 바 "VAT 포함") → **문의**(이메일 한 줄) → **FAQ**. 유료 카드의 **"노출 화면 미리보기" → 풀스크린 모달**(`components/pricing/exposure-preview.tsx`, client): PC/모바일 토글(뷰포트 기본) + 상품별 **전체 페이지 장면 캐러셀**(footer 제외). 상세 SPEC `/pricing`.
