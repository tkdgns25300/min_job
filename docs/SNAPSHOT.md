...
>
> ▶ **2026-09-06 GA4 도입 — 측정 인프라 0에서 이벤트 9개로**: 유료 노출의 값·자리를 다시 볼 근거가 없었다(09-02 재기획 "측정 인프라 0이라 성과형은 후보가 아님"). 콘솔(속성·스트림·향상된 측정 7·보관 14개월·맞춤 측정기준 8)은 운영자가, 코드는 `lib/analytics`(닫힌 이벤트 union + `track`) · `components/analytics/`(로더 · `<TrackEvent>` · `<TrackedLink>`) · 아홉 지점 배선. **핵심 지표 = `source_click`**(원문으로 나감 = 지원하러 감 — 연락처 클릭은 가드레일 #3으로 세지 않는다). 전부 클라이언트에서 보내고 `/admin`과 등록 미리보기는 제외. 함께 바뀐 것: **등록·수정·인증 신청 액션이 `redirect` 대신 결과를 돌려준다**(호출부가 토스트 "공고를 등록했습니다."·"인증 신청을 접수했습니다." + 계측 + 이동 — 08-27 판정 화면과 같은 함정을 마저 걷었다) · 개인정보처리방침 **제3·6·7조 개정 + 시행일 09-06**(= 동의 버전 승격 · ⚠️ 방침 자체의 "7일 전 공지" 조항보다 빨리 시행하게 된다 — 이용자 0명 시점이라 감수할지, 시행일을 미룰지는 **운영자가 정한다(미결)**). 정본 = `docs/ANALYTICS.md`. Vercel env는 넣었고 내부 트래픽 제외는 안 한다(운영자 결정). ⬜ 배포 뒤 첫 클릭이 들어오면 `source_click`에 주요 이벤트 ★.
>
> ▶ **2026-09-06 탭 제목 규칙 + 공유 카드 재설계**: ① `<title>`이 다섯 패턴이었다(홈 `MinJob — …` / 일반 `… | 민잡` / 목록·랜딩은 SEO 꼬리 / 상세는 크롤 제목 / 운영자 `| 민잡 운영자`). 루트 layout `title.template = "%s | 민잡"`(운영자는 admin layout이 덮음)로 **접미사를 한 곳**에 두고 26개 페이지에서 손으로 붙인 접미사를 뺐다. 홈은 브랜드를 한글로(`민잡 — 사역자 청빙 공고`), 목록·랜딩은 꼬리를 떼고 화면 이름만(`서울 사역자 청빙 공고`). 상세는 공고 제목 그대로 — 동적인 게 정상. ② 공고 OG 이미지가 **같은 말을 세 번** 했다(이미지 큰 글자 = 제목, 카톡이 아래에 제목·설명을 또 붙임; "사례비 협의 · 상시모집"은 73%·89%가 같은 글자). 이미지는 **구조**만 — 교회·교단 / 자리 한 줄 / 지역·사례비·마감 칸 셋(값 없으면 흐리게, 있으면 골드) — 제목은 빼고, `og:description`은 **본문 첫 80자**로(`jobShareCard`·`shareDescription` · 테스트 9). 첫 시안의 채움 배지처럼 "싼마이"가 안 되게 칸은 얇은 구분선만. 확인: 실서비스 공고 둘의 PNG(협의·상시 케이스 / 월 150만원·9/21 케이스). 같은 날 밤 **글자를 키웠다** — 카톡 모바일은 1200px을 450px로 보여줘 라벨이 9px이었다(운영자 지적). 여백 64/72 → 52/64, 라벨 24 → 32, 값 46 → 66, 자리 68 → 100(14자 넘으면 80), 도메인 글자 삭제. 판단: 직분 "기타"에 부서·직무도 없는 공고는 자리 줄이 "사역자"로 일반적이 된다 — 제목을 끌어오면 다시 겹치므로 그대로 둔다. 푸터 "호스팅 Vercel Inc."는 **법 확인 후 유지**(전자상거래법 제10조①6호 + 시행령 제10조 = 호스팅서비스 제공자 상호, 시행규칙 제7조 초기 화면 표시, 제45조④2호 과태료 500만원 이하).
>
> ▶ **2026-09-06 공고 카드 배지 — "다 똑같아 보인다"에 대한 답**: 운영자가 카드가 밋밋하고 전부 같아 보인다고 했다. 실데이터로 원인을 짚었다 — 모집중 970건 중 사례비 금액 **27%**(73%가 "협의"), 마감 **11%**, 교회 연결 **0%**(로고·사진 불가). 즉 카드가 같아 보이는 건 **다른 점을 아무것도 안 보여줘서**다. 답은 이미지·색이 아니라 **그 공고에만 있는 사실을 배지로**: 사택 제공(40%) · 마감 임박 D-n(7일 이내, 2%) — 카드의 절반에만 붙어 그 자체가 구분이 된다(인디드 모델). 규칙은 `lib/job-badges`(순수 · 테스트 9), 그리기는 `components/job/job-badges`(client — D-n은 "오늘"이 필요해 마운트 뒤 · `RelativeTime`과 같은 이유). **카드 4종 공용**: 격자(`JobCard`) · 목록 로우(`JobRow`) · 교회 페이지(`OpenJobCard`) · 저장 목록(마이페이지 자체 `JobRow` — 기존 "마감" pill을 공용 배지로). 시안 첫 판은 채움색·점선 pill이었고 운영자가 "싼마이"라 했다 → **한 모양**(얇은 외곽선·작은 라운드·채움 없음), 색은 마감 임박에만 골드. "새 공고" 배지는 뺐다 — 게시 시각의 색(`highlightWithinDays` 3일)으로 대신하되, **최신순 목록에는 강조하지 않는다**(1페이지 전부가 3일 이내라 장식이 된다). 목록 로우 제목은 폰에서 두 줄. 배지 기준 일수는 `constants/domain`(`DEADLINE_SOON_DAYS`·`FRESH_POST_DAYS`). 데이터 변경 0 — 카드 타입에 이미 있는 칸.
>
> ▶ **2026-09-05 (밤) 반응형 전수 점검 — MVP 전 마지막**: 30화면(공개 13 + 교회 7 + 운영자 9 + 로그인) × 8폭(320·390·430·640·768·1024·1280·1536 — 각 브레이크포인트 구간의 **가장 좁은 쪽**을 골랐다; 639·767처럼 "경계 바로 아래"는 그 구간에서 가장 넉넉한 폭이라 뺐다) = **165건 측정, 페이지 가로 넘침 1건**(320px 운영자 단건 검수 — `<fieldset>`의 브라우저 기본 `min-width: min-content`). 수치가 못 잡고 **눈으로 잡은 것 3건**: 320px 헤더 pill "교회 공고 등/록" 두 줄 꺾임 → 줄바꿈 금지 + 360px 미만만 "공고 등록"(`globals.css`에 **`xs` 브레이크포인트(22.5rem)**를 이름으로 정의해 `xs:`로 — 클래스에 `min-[360px]:`처럼 숫자를 박지 않는다 · 모바일 메뉴에 그 진입점이 없어 pill을 숨길 수 없다) · `/jobs` 밴드 "342 / 건" 단위 고아 → 숫자+단위 한 덩이 · 운영자 표 셋(노출 원장·공고 관리·교회 인증)이 320에서 제목 칸이 한 글자씩 세로로 부서짐 → `min-w-2xl`로 부서지는 대신 가로 스크롤(운영자 홈 표의 `min-w-md`와 같은 처방). 터치: 공고 등록 폼 체크 목록 상자가 18px뿐이라 ::before로 34px 영역 + 이름 글자 클릭도 토글. 시트 셋(필터·모바일 메뉴·요금 미리보기)은 390에서 열어 폭·넘침 확인. **판단만 남긴 것**: 320px 목록 카드 제목이 5~6글자만 보임(우측 사례비·저장 칸이 90px) · `/admin/jobs`가 전 공고를 한 표에 그려 138,000px(페이지네이션 없음 — 운영자 도구라 둔다). 🔴 **도구 교훈 둘**(둘 다 시간을 크게 먹었다): ① `next dev`가 떠 있는 채로 `npm run build`를 돌리면 dev 산출물이 지워져 **전 경로 500**이 난다 — 빌드 전에 dev를 내리고 뒤에 다시 띄운다. ② iframe 하네스는 크롬이 **화면 밖·백그라운드 프레임의 인라인 스크립트를 미룬다**(실측 84초) — PPR 조각(`<template id="B:">`+`<div hidden id="S:">`)이 그대로 갇혀 본문이 비어 보인다. React의 `$RC`를 하네스에 옮겨 서버가 준 조각을 직접 꽂고(React 19.2는 시작 주석이 `$~`), 측정 중인 프레임만 화면 위에 남겼다. 하네스는 임시 파일이라 지웠고 사본은 세션 스크래치에만.
>
