...
**완성 (mock + 섹션 확정)**
- ✅ **홈 `/`** — 히어로(중앙, 검색 오버레이+스탯2) · 추천(AD) · 최신 공고. ⚠️ **곧 새 디자인으로 재설계**(§9)
...
- 색: 현재 **neutral(shadcn zinc/oklch, 라이트 전용)**. → **다음 작업 = 딥그린+골드 테마 토큰화**(§9)
...
## 9. ▶ 다음 할 일 (여기서 이어서) — 홈 재디자인

**방식**: 섹션 구조 먼저 → 컴포넌트 → 조합. **색은 토큰이라 나중에 갈아끼워도 구조 안 바뀜.**

**채택 디자인**(사용자가 Claude-design으로 만든 시안 승인): **딥그린 배경 + 골드 포인트.** 잡코리아식 이미지-과밀(=세상적) 회피, 이미지 없이 완성도(아바타·칩·깊이·아이콘·섹션 밴드).

**홈 새 구조 (확정)** — 홈 메인은 **카드 그리드 아님 → 랭킹 리스트**:
1. **헤더**(딥그린): 로고(골드 "Min"+화이트 "Job") + nav(공고·교회·광고안내·소개) + 로그인·공고등록(화이트 pill)
2. **히어로**(딥그린 풀블리드): eyebrow("한국교회 사역자 청빙 플랫폼") + 큰 헤드라인 + 부제 + 검색바(흰 배경) + 스탯 3개
3. **메인 2단**:
   - 좌 **청빙 랭킹 리스트**: 탭(전체/전임/파트) + 로우(번호·교회 이니셜·교회명·지역·AD뱃지·제목·직분/부서/고용 칩·사례비(그린)·N일전). **정렬 = 대표광고(AD) 우선 + 최신.**
   - 우 **사이드바**: **추천 검색어(운영자 큐레이션)** 카드 + **교회 CTA 카드**(딥그린 "우리 교회도 청빙 중인가요? 공고 등록하기")
4. **하단 3-피처**: 한 곳에 모아보기 · 구조화된 비교 · 재공고 추적 (아이콘+제목+설명)

**목업과 다르게 조정(반영 확정)**: 사례비 높은 공고 사이드바 **제외**(세상적) → 추천 검색어로 대체 · 인기검색어→**추천검색어 큐레이션**(로그 없음) · eyebrow **사역자**(부교역자 아님) · "2개 교단"→**"예장 합동·통합 중심"**으로 순화 · "매일 업데이트" 문구 완화(사람 수집).

**구현 순서**:
1. `globals.css` — **딥그린+골드 디자인 토큰**(색 hex 확정 필요) + 라운드·그림자·타이포 스케일
2. 홈 컴포넌트: 히어로 → 랭킹 리스트 로우 → 사이드바(추천검색어·교회CTA) → 3-피처
3. 그 뒤 톤을 `/jobs`·상세로 확산 + **공고 상세 사이드바 레이아웃 재고**(핵심조건 눈에 띄게)

**미결(다음 세션 초반 정할 것)**: 딥그린/골드 정확한 hex · 랭킹 리스트 로우 최종 컴포넌트 · 추천 검색어 큐레이션 소스(상수?).

> 참고 시안 이미지는 대화에만 있음(scratchpad 저장 안 됨). 다음 세션에서 필요하면 사용자에게 재공유 요청.
