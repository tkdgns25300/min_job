...
| 수익화/결제 | ~90% | ✅ **KCP 가맹·카드사 심사 둘 다 통과 → 실카드결제 활성(2026-08-05)**. 인증 교회는 결제 화면에 도달한다(2026-08-27 멤버십 배선). ✅ **3등급 자리·상품표 구현(2026-09-03 · prod)** · ✅ **결제 마무리 코드(2026-09-03 · dev)** — 주문 저장·정원 판정·`featured_*` 적용·모바일 복귀·운영자 원장. 남은 것 = `featured_from` 마이그레이션 적용 → 실결제 1건 확인 → prod(§7 순서 4) |
...
4. ~~결제 마무리~~ ✅ **코드 완료(2026-09-03)** — 상품 3등급 전환 · 자리 구현 · 비슷한 공고 규칙 교체(묶음 1 · prod 배포) / 주문 저장 + 정원 판정 · 실 노출 적용(`featured_tier`·`featured_from`·`featured_until`) · 모바일 복귀 · 운영자 노출 카드·원장 표(묶음 2·3 · **dev만**). 답한 것 — 광고 중 공고가 마감되면 남은 기간 소진, CANCELLED = 게재 전 전액 취소, REFUNDED = 게재 후 예외 환불. **← 지금 여기: ① `featured_from` 마이그레이션 적용 ② `types/database.ts` 재생성(diff 0이어야) ③ 시드 E2E ④ 실결제 1건(테스트 채널 또는 기본 1주 + 환불) ⑤ prod 머지**
...
### H. 노출 결제 flow (`/mypage/church/promote`, 2026-07-20 — **결제 마무리 2026-09-03**: 3등급 · 시작 주 · 정원 · 주문 저장 · 적용 · 모바일 복귀)
- **공개 `/pricing`은 "안내+문의" 유지**, **실 결제는 인증 교회 전용 `/mypage/church/promote`로 분리**. 진입 = `/mypage/church` 사이드바 "노출 신청 →".
- 화면 = 대상 공고 → 노출 상품(기본·플러스·스페셜 + 그 주 남은 자리) → 시작 주(이번 주·다음 주) → 기간(1·2·4주, 묶음가 표) → 결제 요약 → 약관 동의 → 결제 → 결과 3갈래(적용됨 / 청구됐는데 적용 못 함 / 청구 없음). 정본 = SPEC 마이페이지 절.
- **PortOne V2(KCP CARD)** 결제창 → **Server Action `completePromotion`**(2026-09-03 — 옛 route handler `/api/payments/complete` 삭제)이 PortOne 조회로 상태·금액을 대조하고, 주문은 `customData`에서 다시 읽어 정원·겹침을 재확인(경합이면 전액 취소)한 뒤 원장 INSERT(멱등) + `featured_*` 적용 + `updateTag("jobs")`. 모바일은 `?paymentId=` 복귀가 같은 액션을 부른다. 가격 단일 소스 = `EXPOSURE_PRODUCTS`.
