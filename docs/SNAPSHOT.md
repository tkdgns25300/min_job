...
| 수익화/결제 | ~60% | ✅ **KCP 가맹·카드사 심사 둘 다 통과 → 실카드결제 활성(2026-08-05)**. 인증 교회는 결제 화면에 도달한다(2026-08-27 멤버십 배선). ✅ **상품 구성 재기획 확정(2026-09-02 — 자리 셋 · 사다리 3등급 · §9)**, 코드 전환은 결제 마무리와 함께. 그 뒤 주문 저장(`job_promotions` INSERT)·실 노출 적용(featured)·모바일 redirect 복귀(§7 순서 4·마지막) |
...
**상태 한 줄**: 읽기는 전부 실 DB(모집중 953건 · 검수 큐 46건 · 게시판 31개 수집 중), 쓰기는 8개 중 6개 완료(검수 판정 · 공개 공고 저장 · 교회 인증 접수·판정 · 교회의 공고 등록·수정 · 교회 정보 · 북마크). 공개 화면은 8/29~30에 한 차례 다듬었다(카드 사례비 · 상세 정리 · 페이지네이션). **남은 쓰기 = 노출 결제 주문 저장**(클레임은 2026-09-01 완료).
...
4. **결제 마무리 ← 지금 여기** — 3에서 정한 상품으로: 상품 3등급 전환(상수·enum 마이그레이션·요금/결제 화면) · 자리 구현(홈 3칸 · 목록 로우 5줄 · 연관 첫 칸) · 비슷한 공고 규칙 교체 · 주문 저장(`job_promotions`) + 정원 판정 · 실 노출 적용(`featured_*`) · 모바일 redirect 복귀. 체크리스트 = ROADMAP 2-3·1-8. 그때 답할 것 하나 — 광고 중 공고가 주중 마감되면 남은 기간을 어떻게 하나(REFUNDED↔CANCELLED 경계)
...
### C. 민잡 상품표 (확정 2026-09-02 · 이전 2단은 2026-07 초안)

| 등급 | 자리 | 정원(주) | 1주 | 2주 | 4주 |
|---|---|---|---|---|---|
| **기본 공고** | 최신순 리스트(무료) | — | **0원** | | |
| **기본** | 연관 첫 칸(같은 지역·자격 문 통과 상세 페이지) | 없음 | **29,000** | 55,000 | 89,000 |
| **플러스** | 목록 상단 로우 + 연관 첫 칸 | **2** | **49,000** | 94,000 | 149,000 |
| **스페셜** | 홈 카드 + 목록 상단 로우 + 연관 첫 칸 | **3** | **99,000** | 189,000 | 299,000 |
...
히어로 → **상품 4카드**(무료·기본 2.9만·플러스 4.9만·스페셜 9.9만/주 — 문구·비교표는 `EXPOSURE_PRODUCTS`에서 파생, 가격 공개 + "문의하기") → **한눈에 비교** → **믿고 노출하세요**(getCoverageStats 실집계 + 하단 슬림 바 "운영자 검수·VAT 포함") → **문의**(mailto) → **FAQ**. 유료 카드의 **"노출 화면 미리보기" → 풀스크린 모달**(`components/pricing/exposure-preview.tsx`, client): PC/모바일 토글(뷰포트 기본) + 상품별 **전체 페이지 장면 캐러셀**(footer 제외). 상세 SPEC `/pricing`.

### H. 노출 결제 flow (`/mypage/church/promote`, 2026-07-20 — 상품 3등급 2026-09-03 · ⬜ 정원·시작 주·주문 저장은 1-8)
