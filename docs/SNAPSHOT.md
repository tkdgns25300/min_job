...
| 수익화/결제 | ~60% | ✅ **KCP 가맹·카드사 심사 둘 다 통과 → 실카드결제 활성(2026-08-05)**. 인증 교회는 결제 화면에 도달한다(2026-08-27 멤버십 배선). ⚠️ **상품 구성은 2026-07-08 초기 계획 그대로 — 실배포 전 재기획(§7 순서 3)**. 그 뒤 주문 저장(`job_promotions` INSERT)·실 노출 적용(featured)·모바일 redirect 복귀(§7 순서 4·마지막) |
...
**상태 한 줄**: 읽기는 전부 실 DB(모집중 953건 · 검수 큐 46건 · 게시판 31개 수집 중), 쓰기는 8개 중 6개 완료(검수 판정 · 공개 공고 저장 · 교회 인증 접수·판정 · 교회의 공고 등록·수정 · 교회 정보 · 북마크). 공개 화면은 8/29~30에 한 차례 다듬었다(카드 사례비 · 상세 정리 · 페이지네이션). **남은 쓰기 = 클레임 · 노출 결제 주문 저장**, 교회 행은 아직 0곳(클레임이 열어야 `/churches/[id]`가 생긴다).
...
2. **소개 페이지(`/about`) 재작성** — 지금 없는 것이 "공고가 어디서 어떻게 오나"다: 공개 게시판 31곳 목록(`SOURCE_BOARDS` 그대로) · 자동 수집 → AI 정리 → 자동 공개/운영자 검수 · 원칙(원문 재게시 없음·요약+출처 링크·지원용 연락처만) · 교회 담당자께(내려달라·고쳐달라 경로 = opt-out, 직접 등록·인증). 소개이자 **가드레일 #1의 방어선**이라 베타 전 필수(반나절)
3. **BM·노출 상품 재기획** — §9는 2026-07-08 초기 계획이다. 실배포 전이라 **어떤 상품으로 갈지**(종류·노출 위치·기간 단위·가격·묶음)를 다시 정한다. 모델 자체(무료 공고 + 유료 노출)는 그대로, 상품 구성이 재검토 대상 → ROADMAP 2-3
4. **결제 마무리** — 3에서 정한 상품으로 주문 저장(`job_promotions`) · 실 노출 적용(`featured_*`) · 모바일 redirect 복귀. **마지막**(운영자 결정 — 상품이 정해진 뒤에 붙인다)

**❓ 클레임 — 미정(2026-08-30 · 운영자가 아직 결정하지 않았다. 아래는 검토 의견이다)** — 베타 사용자(구직자)에게 주는 것이 없고, 필요 조건(인증 교회·"우리 공고예요" 문의·크롤 공고에 낼 광고비)이 전부 0이다. **광고도 클레임을 요구하지 않는다**: 교회가 직접 등록해서 광고하는 쪽이 내용도 정확하고 동선도 짧다(인증 → 등록 → 광고, 지금 코드로 된다). 남는 과제는 **중복 정리 하나** — 직접 등록(특히 유료)이 들어오면 같은 자리의 크롤 공고를 그날 마감한다(`/admin/jobs` 마감, 있는 기능). ⚠️ 크롤러의 중복 판정은 `review_data` 초안끼리만 보고 교회 직접 등록분은 안 본다 — 인증 교회가 늘면 "직접 등록이 들어오면 같은 교회명·지역의 크롤 공고를 운영자 홈에 띄우기"를 붙인다. 클레임이 필요해지면 **셀프서비스가 아니라 운영자 연결부터**: `/admin/jobs/[id]` 편집에 "교회 연결" 한 칸(인증 교회 select → `jobs.church_id`) — 교회가 링크를 보내고 운영자가 인증 서류와 대조해 연결한다(자동 매칭은 교회명이 흔들려 못 믿고, 사람은 쉽다).
...
- 캐시(빌드 출력, 2026-07-29 측정): 홈·/jobs·/jobs/[id]·/churches/[id]·/about·/pricing·/terms·/privacy = **`◐ PPR`**(이전 `○ Static`에서 바뀜 — 헤더 계정 영역이 세션을 읽는 dynamic hole이라 셸은 계속 prerender되지만 문서 응답은 `no-store`) · `/login`·인증(/mypage·/mypage/church·/jobs/new·edit) = `◐` · **`/admin/jobs`는 `○ Static` 유지**(admin 셸에 공개 헤더가 없음). **`/admin`(홈)은 2026-08-25 재설계로 `◐`**(검수·인증 큐와 `crawl_run`이 캐시 불가). **`/admin/review`·`/admin/review/[id]`·`/admin/review/[id]/group` = `◐`**(미검수 데이터 + `requireOperator`, 2026-08-22 측정) · 삭제된 `/admin/ingest`는 `○ Static`이었다. ⚠️ admin 셸의 `usePathname`은 **동적 세그먼트에서 프리렌더할 수 없어** `<Suspense fallback={<AdminSidebarFallback/>}>`로 감쌌다(스켈레톤이 아니라 강조 표시만 없는 같은 사이드바) — 이 경계가 없으면 `/admin/review/[id]` 빌드가 막힌다. 경계를 넣어도 `/admin/jobs`는 `○ Static`을 유지한다 · `/auth/callback`·`/api/payments/complete`·`/opengraph-image` = `ƒ` · **`/sitemap.xml`·`/robots.txt` = `○ Static`**(sitemap은 `cacheLife("days")`에서 온 `1d/1w` revalidate) · `ƒ Proxy` 존재. 공고 데이터 자체는 계속 `'use cache'`+`cacheTag`+`cacheLife`.
...
>
> ⚠️ **2026-08-30 — 실배포 전 상품 구성 재기획 예정(§7 순서 3 · ROADMAP 2-3).** "되돌리지 말 것"은 **A의 모델 판정**(무료 공고 + 유료 노출, 성공보수 제외)에 대한 말이다. B 이하의 **상품 레버·종류·가격·기간 단위**는 코드 전 초기 계획이라, 실데이터(모집중 953건 · 교회 0곳 인증)와 결제 활성 뒤의 눈으로 다시 정한다. 결제 마무리(주문 저장·노출 적용)는 그 뒤에 붙인다.
