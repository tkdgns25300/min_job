...
>
> ⚠️ **이 스냅샷 이후 변경(아래 본문보다 라이브 문서가 정확)**: 홈 검색 오버레이 추가 · **교회 규모(대/중/소) 전면 제거** · 직분에 **담임목사 추가** · 홈 스탯 "청빙 중 교회" 제거 · 대외 용어 **"사역자 청빙"으로 통일**. 규모·직분·카피 관련 본문은 옛 상태이니 CLAUDE·SPEC·DATA·ROADMAP을 우선한다. (전면 재생성은 디자인 단계 후 체크포인트에)
...
- **창립연도(`foundedYear`) 추가**(nullable). 담임목사 이름·전체주소·예배시간은 교회 필드로 **미채택**(개인정보 경계·수집 편차·지도 Phase 2). ※ 직분 enum의 담임목사와 무관.
- **소유권 인수(claim) 기능 제거.**
