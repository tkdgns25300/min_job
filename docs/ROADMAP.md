...
- [x] ✅ **디버깅 묶음 (운영자 리스트 7건 · 2026-08-29)** — ① `/jobs` 페이지네이션 윈도우(47개 전부 나열 → 현재 ±1·첫·끝·줄임표·이전/다음) ② 검수 `고치기`: 코드상 동작했지만 회색 글자가 버튼으로 읽히지 않았다 → 테두리 버튼 + **값을 눌러도 펼쳐짐** ③ 묶음 판정의 모든 멤버에 **검수 화면 ↗**(한때 공개된 건만 링크) ④ 검색이 통짜 substring이라 두 단어("방주교회 전임교역자")가 0건 → **단어 AND** + 플레이스홀더에 "공고 제목" ⑤ 중복 처리 메모 선택(비면 “묶음 판정 — 같은 자리”) ⑥ 운영자 홈 실패 게시판을 **링크**(`boardUrl` = 크롤러 `sources.json` 사본) + 에러 문구 `title` ⑦ 교회 직접 등록 공고의 상세에 원문 링크 대신 **배지**. ⚠️ ②는 운영자 환경에서 "고치기를 눌러도 반응 없음"으로 보고됐는데 **로컬에선 네 조합 모두 재현 안 됨** — UX를 고쳤고, 계속 안 되면 브라우저·URL·콘솔 확인이 필요하다.
