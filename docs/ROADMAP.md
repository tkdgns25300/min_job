...
- [x] 소개 (`/about`) — 정적. ✅ **재작성 2026-08-31~09-01(8→6섹션)** — "공고는 이렇게 모입니다"(두 경로 · **직접 등록 먼저** — 지향점) + 교회 담당자 카드(**인증 CTA만** — 수정·삭제 안내는 문의 페이지로 미룸, 운영자 2026-09-01) 신설, "이런 분들께"·독립 문의 섹션·혜택 ③(재공고 이력 — 보류라 거짓)·FAQ "내리고 싶어요" 삭제, "조건"→"자리", 현황은 **공고→지역→교회 3수치**(교단 제외). **top nav 승격**(header `NAV_LINKS`에 소개 추가). 크롤링·자동수집 단어 미노출 프레임 유지(구성 정본 = SPEC 소개 절)
  > ✅ **"왜 만들었나요" 신설(2026-09-05 · 7섹션)** — 만든 사람의 동기·신앙 배경(예장합동 교회 출석) + **이단·사이비 거절 원칙**(수집 단계 필터 · 교회 직접 등록은 증빙으로 실체 확인 · 놓친 건은 이메일 신고). 이 페이지 유일한 1인칭 구획. 함께: 히어로에서 "여기저기 흩어진" 삭제 · "무엇이 좋은가요" 두 줄 어미 통일 · FAQ "바로 고칩니다" → "수정하겠습니다". 근거 = SNAPSHOT 2026-09-05
...
- [x] ✅ **스페셜 값 인하(2026-09-05)** — 9.9 → **6.9만/주**(2주 12.9 · 4주 20.9만). 갓피플 스페셜에 값을 맞추던 근거를 버렸다 — 트래픽 실적 0에 같은 급의 값은 첫 판매를 막는다(SNAPSHOT 2026-09-05). 비율 3.4:1.7:1 → 2.4:1.7:1
