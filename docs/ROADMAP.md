...
**▶ 중복 판정: 교회 직접 등록 ↔ 크롤링 (2026-08-05 식별 — 미설계)**
...
> **빠진 것**: 교회가 MinJob에 직접 올린 공고와, 크롤러가 그 교회 게시판 글을 수집한 공고가 **같은 공고일 때**. 출처가 달라 크롤러 내부 dedup으로 안 잡힌다. 크롤 데이터가 들어오는 순간부터 쌓이므로 **유입 전에 정해야 한다.**
