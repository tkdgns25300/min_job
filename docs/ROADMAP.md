...
- [ ] **공개**: `/`(홈) · `/jobs`(검색·필터·정렬·페이지당 개수 동작) · `/jobs/[id]`(재공고 접이식·비슷한 공고·링크) · `/churches/[id]`(갤러리 라이트박스·채널 링크)
- [ ] **인증**: `/login`(폼 제출·간편 버튼·`?next=` 분기) · `/mypage`(+`/church`·`/church/info`·`/church/promote`·`/verify` — 상태 분기·탭·케밥·결제 flow) · `/jobs/new`·`/edit`(위저드 스텝·검증·기타 입력)
- [ ] **admin**: `/admin`·`/jobs`·`/verify`·`/ingest`(탭·필터·시트·구조화·승인/반려 인터랙션)
- [ ] **공통**: 헤더 계정 위젯 분기 · hover/포커스/키보드 · 빈 상태·로딩·에러 UI · 모바일 반응
> 순회하며 발견한 구멍은 이 아래에 페이지별로 목록화한다(발견 시 추가).
