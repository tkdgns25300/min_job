...
- [x] **교회 인증 신청 접수 (2026-08-25)** — 화면만 있던 `/mypage/verify`에 실 배선. **판정은 운영자가 DB에서 직접** 하므로 승인·반려 액션은 만들지 않았다.
  ...
  - **폼은 `useActionState`** — JS 없이도 제출된다(로그인 폼과 같은 선택). ⚠️ `"use server"` 파일은 **async 함수만 export**할 수 있어 초기값 상수를 그쪽에 두면 **빌드는 통과한 뒤 요청에서 터진다**(실측). select 3개는 "선택하세요"가 기본값 — 첫 옵션이 자동 선택되면 안 건드린 사람의 교회가 전부 `예장합동`·`서울`·`담임목사`가 된다.
  - **`experimental.serverActions.bodySizeLimit = 12mb`** — 기본 1MB라 그대로 두면 스캔 PDF가 대부분 거부된다. 전역 설정이다.
  ...
  - 검증: 순수 규칙 **46건**(정규화 표기 4종·경계·닫힌 enum 밖·전화 표기 다양성·파일 10MB 경계) + **JS 없는 폼 제출을 그대로 재생**해 거부 5건(동의·서류·공백·자릿수·직분) · 신규 접수 · 재신청(옛 서류 삭제 확인) · 액션 가드 3건(PENDING·APPROVED·미로그인) · DB CHECK 재확인. 임시 계정·교회·서류는 전부 삭제했다.
