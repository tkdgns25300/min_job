...
- [ ] 🔴 **성공 알림을 토스트로 통일 — 지금 넷이 각자 다르다(2026-08-27 전수 확인)**. 화면에 머무는 성공 지점: ① 교회 정보 저장(인라인 배너) ② 공고 마감·다시 모집(**성공을 아예 안 알린다** — 실패만 표시) ③ `/admin/jobs/[id]` 저장(`done` 상태 문구) ④ `/admin` 공개 목록 새로고침(`done` 상태 문구). 셋 다 손수 만든 것이고 모양·자리가 다르다. shadcn 현재 표준은 **`sonner`**(옛 `toast`/`use-toast`는 폐기) — `<Toaster />`를 레이아웃에 한 번 심고 `toast.success()`를 부른다. ⚠️ 루트 레이아웃 + 4화면을 건드리는 별건이라 교회 정보 커밋과 섞지 않았다.
