...
- [~] **NULL 표시 UI**(스키마를 푼 대가 — 안 하면 모르는 것을 아는 척한다. ~~게시일~~ 은 필수 복귀로 제외). **표시 규칙은 확정·구현 완료**(2026-08-16 교회 조인 정합): 공개 화면은 모르는 조각을 **생략**하고(`format.ts`의 `churchLocation`·`denominationLabel`), 운영자 화면만 "미상"을 명시한다. ✅ **필터 쪽 답도 나왔다(2026-09-04)** — 표시 규칙만 정하고 "미상이 필터에서 조용히 빠지는 것"은 정하지 않았는데, **칩마다 건수를 붙여** 누르기 전에 보이게 했다(SPEC 정렬·필터 절). "정보 없음" 칩·안내 배너는 두지 않기로 했다(근거는 그 절). ⏸ **남은 것은 `denomination_source` 게이팅뿐**(`stated`·`registry`·`operator`일 때만 확정 표시) — 그 컬럼이 DB에 있어야 해서 마이그레이션과 한 묶음: **검수 우선순위는 교단보다 지역**(비면 지역 필터에서 탈락 = 사실상 안 보이는 공고)
...
- [ ] 📮 **크롤러에 전달할 것 — 제목에 적힌 부서·고용형태를 놓친다(실측 2026-09-04).** 공개 중 953건에서 **부서가 빈 643건 중 68건**은 제목에 부서가 그대로 적혀 있다("공군중앙교회 **초등부** 전임사역자를 모십니다"·"포항 흥해중앙교회 … **유치부** 교육전도사"). **고용형태가 빈 541건 중 154건**도 같다("**준전임**전도사를 모십니다"). 본문만 보고 뽑는 것으로 보인다 — **제목도 함께 읽으면** 두 축이 그만큼 채워진다. 필터 칩 건수를 붙이면서(SPEC 정렬·필터 절) 드러난 결손이고, 화면에서 고칠 수 있는 게 아니다.
  > ⛔ **부서 enum에 값을 끼워 넣는 방식은 반대다.** 제목에 "교육전도사"만 있고 연령이 없는 80건은 **교회가 아직 부서를 안 정한 것**이라 비어 있는 게 맞다(직분·검색으로 이미 찾을 수 있다). "교육부"를 유초등부 옆에 두면 **상위·하위가 섞여**(교육부 ⊃ 유초등부·중고등부) 서로를 못 보게 되고, 기타로 넣으면 지금 기타가 뜻하는 특수 사역(선교사·장애인부·방송간사 11건)이 묻힌다. 층을 다루려면 아래 **담당부서 재설계** 항목에서 상위·하위를 함께 정해야 한다(2026-09-04 검토).
- [ ] 📮 **크롤러에 전달할 것 — `jobs`에 첨부 파일 이름 칸(2026-09-05).** 원문 게시판의 첨부(이력서 양식 등)는 `source_data.attachments`에만 있고 `jobs`에 없다. 공개 953건 중 양식 첨부(이미지 아닌 파일)가 있는 건 **86건**인데 공개 화면은 알 길이 없어, 지금은 수집 공고 전부에 "있으면 원문에서" 한 줄을 붙여 둔다(SPEC 공고 상세 절 · 임시). 크롤러가 공개할 때 **파일 이름 배열**(예: `attachment_names text[]`)을 함께 쓰면 그 줄이 "있는 공고에만 · 이름과 함께"가 된다. 기존 952건은 `source_data`에서 한 번 옮기는 SQL로 채운다(우리가 쓰고 운영자가 실행). 연락처 4칸을 만들 때와 같은 방식. ⛔ 공개 화면이 `review_data`·`source_data`를 직접 읽는 길은 검토 끝에 접었다 — 그 표들은 크롤러 작업대라 모양·보존이 우리 통제 밖이다.
...
- [ ] ⏸ **`/jobs` 사역직/일반직 필터축이 없다 (2026-08-21 발견 · 보류 결정)** — SPEC은 **"기본뷰=사역직(`job_kind @> ARRAY['MINISTRY']`), 일반직은 필터 전환, 혼합 공고는 양쪽에 다 뜬다"** 로 확정했는데(2026-07-28 운영자 확정 · SPEC §목록·§필터표) **코드에 그 축이 없다** — `FilterDim`에 `jobKind`가 없고 `JobCard`에 `jobKind`도 없다. mock에 일반직이 0건이라 안 보였고, 위 항목에서 반례를 심자 **기본 목록에 순수 일반직 2건이 그대로 섞였다.**
  ...
  > 🔴 **이 축이 없어서 한 화면이 같은 라벨에 두 숫자를 쓴다(2026-09-04 발견)** — 필터 칩은 "서울 223"(일반직 포함)인데 같은 페이지 아래 랜딩 허브는 "서울 207"(랜딩은 사역직만)이다. 실측 차이 16건 = 서울의 일반직 공고 수(OPEN 225 = 사역직 209 + 일반직 16). 두 수 각각은 자기 도착지를 정확히 말하지만 **나란히 놓이면 데이터를 못 믿게 된다.** 이 축이 들어와 기본 목록이 사역직이 되면 두 수가 저절로 맞는다 — 그래서 여기서 고칠 일이고, 칩이나 허브에서 숫자를 떼는 것은 증상만 가린다.
...
- [ ] **법령 신고 — 직업안정법 `직업정보제공사업` (2026-09-04 확인)**: 인터넷으로 구인정보를 제공하고 광고비를 받으므로 **신고 대상으로 보인다**(직업안정법 제2조의2 정의: "컴퓨터통신 등으로 구인·구직 정보 등 직업정보를 제공하는 사업"). 미신고 시 **1년 이하 징역 또는 500만원 이하 벌금**. 신고는 무료·7일, 관할은 **주된 사업소 소재지** = 안동(대구지방고용노동청 안동지청 · 안동고용복지플러스센터 054-851-8061). **2026-09-04 문의 접수, 회신 대기.**
  - 우리는 **직업소개가 아니라 정보제공**이다 — 사람을 추천하거나 이력서를 대행하지 않는다(사람인=정보제공 신고 / 원티드=유료직업소개 등록 / 알바몬=둘 다. 실물 푸터 확인). ⚠️ "지원하기"로 이력서를 대신 보내거나 교회에 사람을 추천하는 기능을 붙이면 **등록 대상으로 넘어간다** — 기능 추가 시 이 선을 먼저 본다
  - 신고 후 할 일: **신고번호 표시**(시행령 제28조 제5호 · "정보이용자들이 알아보기 쉽게"). `constants/business.ts`에 칸 하나 추가 → 푸터·약관 자동 반영(통신판매업 번호와 같은 구조)
  - 함께 물어볼 것: ① 자택 사업장의 **사무실 전용면적 10㎡** 기준 ② 푸터의 우리 주소·전화(시행령 제28조 제2호는 사업자 연락처 기재를 금지하는데 전자상거래법은 표시를 의무화 — 두 법이 부딪히는 지점) ③ 원문에 연령 제한이 있거나 사례비가 최저임금에 못 미치는 공고를 그대로 게재해도 되는지(교역자는 근로자성 자체가 다투어진다)
...
- [x] **OG 이미지**(2026-07-30) — `app/opengraph-image.tsx`로 1200×630 브랜드 카드 생성(딥그린 + 골드 로고 + 도메인). 전 페이지 타입에서 `og:image` 1개씩 확인. `SITE_OPEN_GRAPH.images`로 지정 — ⚠️ openGraph를 재정의하는 상세 페이지는 **파일 기반 이미지도 상속받지 못하기** 때문
  ...
  - [x] **공고별 이미지**(2026-09-05) — `(public)/jobs/[id]/opengraph-image.tsx`. 세 줄: 교회·교단·지역 / 자리(직분·부서 — 직분이 "기타"뿐이면 교회가 쓴 제목) / 사례비·출근·마감. 줄의 내용은 `lib/seo` `jobShareLines`가 정하고 **`og:description`도 같은 줄**을 쓴다(그림과 글이 다른 말을 하지 않게). 검색 스니펫용 `description`은 교회 문장 그대로.
    - **글꼴**: `PretendardVariable.woff2`를 굵기 700으로 고정해 **KS X 1001 2,350자 + ASCII + 문장부호**만 남긴 `fonts/pretendard-bold-subset.ttf`(447KB). 만드는 법은 `scripts/subset-og-font.py`(로컬 python venv에 fonttools·brotli). 2,350자 밖 음절은 썸네일에서 빈칸 — 실데이터 452자 중 1자('뱡')가 밖이라 `--also`로 넣었다. 새 글자가 보이면 다시 만든다.
    - ⚠️ **`openGraph.images`를 손으로 적지 않는다** — 빌드가 라우트에 해시를 붙여(`/jobs/[id]/opengraph-image-1y46gx?…`) 손으로 쓴 주소는 404였다. 세그먼트의 파일 규약이 자동 주입하게 두고, 그래서 상세의 `openGraph`는 `SITE_OPEN_GRAPH`를 통째로 펴지 않고 필드를 골라 쓴다(로고 `images`가 주입을 막는다). `alt`는 정적 export 한 줄(공고별 alt는 `generateImageMetadata`가 필요해 안 함).
...
- [x] ✅ **지역·직분·부서 랜딩 라우트 (2026-09-03)** — `/jobs/region/[region]` · `/jobs/position/[position]` · `/jobs/department/[department]` **28개**(지역 18 · 직분 4 · 부서 6). 각자 H1·title·description·canonical·breadcrumb를 갖고 `generateStaticParams`로 프리렌더(`◐ PPR`, 값이 닫힌 집합이라 가능). 규칙 정본 = `lib/job-facets`(순수 · 테스트 19개). 라우트 파일 셋은 Next가 정적으로 찾아야 하는 것만 갖고 몸통은 `facet-page`가 한 벌로 든다
  ...
  - 발견 경로: sitemap + **공고 상세의 "같은 조건 모아보기"** + `/jobs` 하단 허브. ~~홈 추천 검색어 칩~~ 은 빠졌다(2026-09-05 — 사이트 안 사용자에겐 필터 걸린 목록이 맞아서 `/jobs?{축}=`으로 되돌림)
...
- [x] 소개 (`/about`) — 정적. ✅ **재작성 2026-08-31~09-01(8→6섹션)** — "공고는 이렇게 모입니다"(두 경로 · **직접 등록 먼저** — 지향점) + 교회 담당자 카드(**인증 CTA만** — 수정·삭제 안내는 문의 페이지로 미룸, 운영자 2026-09-01) 신설, "이런 분들께"·독립 문의 섹션·혜택 ③(재공고 이력 — 보류라 거짓)·FAQ "내리고 싶어요" 삭제, "조건"→"자리", 현황은 **공고→지역→교회 3수치**(교단 제외). **top nav 승격**(header `NAV_LINKS`에 소개 추가). 크롤링·자동수집 단어 미노출 프레임 유지(구성 정본 = SPEC 소개 절)
  > ✅ **"왜 만들었나요" 신설(2026-09-05 · 7섹션)** — 만든 사람의 동기·신앙 배경(예장합동 교회 출석) + **이단·사이비 거절 원칙**(수집 단계 필터 · 교회 직접 등록은 증빙으로 실체 확인 · 놓친 건은 이메일 신고). 이 페이지 유일한 1인칭 구획. 함께 카피 정리: 히어로 태그라인 · "무엇이 좋은가요" 두 줄 어미 통일 · FAQ 답("수정하겠습니다"). 근거 = SNAPSHOT 2026-09-05
- [x] 공고 노출 안내 (`/pricing`) — 정적, 3카드(무료·프리미엄·대표광고) + 카드별 노출 미리보기 모달, 가격 공개(앵커) + 결제만 문의 (실제 광고 기능·라이브 결제는 Phase 2)
- [~] 약관·개인정보처리방침 (`/terms`, `/privacy`) — ✅ **2026-09-04 최초 시행분 작성(작업 트리, 미배포 · 시행일 상수 2개 = 09-04, 그전 07-20·08-25 화면은 초안이라 이력에서 뺐다)**: 그전 문서는 6개월 전 시스템을 설명했다("국내 처리·보관" — 실제는 Vercel 미국 실행 / "결제 추후" — 이미 판매 / "관심 공고 단말 저장" — DB로 이전). 방침 14항 = 개인정보보호위원회 작성지침(2025.4.) 순서 · 항목별 [항목][목적][근거][보유] · **공개 게시판 수집 연락처(제20조) 신설** · 국외 이전(제28조의8 ①3호, Vercel) 신설 · 수탁사 실명(Supabase·Vercel·코리아포트원·NHN KCP). 약관 15조+부칙 = 두 수집 경로 · **정보제공자 위치(소개·추천 X — 직업안정법 신고 구분과 한 쌍)** · 결제대행 실명 · **게재 후 일할 환불**(제17조②5호 단서) · 연락처 오용 금지. 운영자 결정(2026-09-04): **① 마감 시 공고 이메일·전화 파기 ② 게재 후 남은 일수 일할 환불(요청 접수일 기준 · 3영업일)**. 사업자정보 `constants/business.ts` 단일 소스(+ 호스팅서비스 제공자 표시 — 전자상거래법 제10조).
  ⚠️ **문서가 코드보다 먼저 약속한 것 — 운영자 결정(2026-09-04)으로 문서를 먼저 시행하고 구현은 todo로 남겼다. 마감 시 연락처 파기 문장(약관 제7조 · 방침 §1③·§2·§10)은 ⓐ가 끝나기 전까지 실제 동작과 다르다**: ⓐ 결정 ① 구현 — `jobs_needs_contact` CHECK를 CLOSED에서 완화 + 마감 액션(교회·어드민 둘)에서 `contact_email`·`contact_tel` NULL + 이미 마감된 공고 일괄 처리 + "다시 모집" 시 연락처 재입력 요구. 크롤 공고의 마감 판정은 크롤러 몫이라 **우리 쪽 일일 배치 또는 크롤러 훅** 중 택일 ~~ⓑ 결정 ② 문구 2곳~~ ✅ 같은 날 반영(`promote-checkout.tsx` · `pricing` FAQ · `domain.ts` REFUNDED 주석) ~~ⓒ 시행일 상수~~ ✅ 09-04. 기존 동의 기록 1건(08-25 버전)은 운영자 본인 교회라 재동의 대상 없음 ⓓ **개인정보취급자 접속기록** — 「개인정보의 안전성 확보조치 기준」 제8조(1년 이상 보관·점검)가 운영자의 PII 열람에 적용되는지 확인, 적용되면 어드민 PII 화면 열람 기록 표 하나(Supabase 로그 보존은 7일이라 못 쓴다). 문서에는 하지 않는 일을 적지 않았다 ⓔ 정식 법률 검토는 그대로 남는다(약관규제법 관점 — 검수에서 이행보조자 면책·위반 시 몰취·묵시적 동의 조항은 표준약관 문구로 이미 고쳤다)
...
- [x] **반응형 전수 점검 2차(2026-09-05 · 30화면 × 8폭 = 165건 · MVP 전 마지막)** — 가로 넘침 1건(운영자 단건 검수 320 · `fieldset` 기본 `min-width: min-content` → `min-w-0`) + 눈으로 3건(320 헤더 pill 두 줄 · `/jobs` 밴드 단위 고아 · 운영자 표 3개 칸 부서짐) + 터치 영역(체크 목록 18px → 34px). 판단만: 320 카드 제목 5~6글자 · `/admin/jobs` 전 공고 한 표(138k px). 근거·도구 교훈 = SNAPSHOT 2026-09-05 (밤)
...
8. [x] **카드사 등록신청 — 심사 종료(제출 2026-07-21 → 종료 2026-08-05)** — `partner.kcp.co.kr` 신용카드 일반결제 등록신청. 제출 시 기재: 심사용 계정 `test1@test.com` + 결제창 경로(`/mypage/church/promote`) + 통신판매 면제 사유. 체크 7항 통과(비회원 구매불가라 #7=아니오+계정 제공, 통신판매는 #3 면제 예외). **심사 중 걸려 있던 "URL·사업자정보·상품/가격 변경 금지" 제약 해제 → prod 배포 동결도 해제.**
   ...
   >
   > ✅ **`/pricing` 카피 전환(2026-09-03)** — CTA "노출 신청하기" → `/mypage/church/promote`(게이트는 결제 화면이 맡아 이 페이지는 세션을 안 읽는다), FAQ 결제 답변. (이전: "온라인 결제는 준비 중"은 멤버십이 붙기 전까지 사실이었다)
...
- [x] ✅ **스페셜 값 인하(2026-09-05)** — 최상단 **6.9만/주**(2주 12.9 · 4주 20.9만). 트래픽 실적이 없는 동안은 벤치마크와 같은 급의 값을 부르지 않는다 — 근거는 SNAPSHOT 2026-09-05
- [x] ✅ **연관 광고 칸 1 → 3(2026-09-05)** — `SIMILAR_AD_SLOTS`(비슷한 공고 6장 중 위 3칸). "첫 칸 하나"는 정원 없는 기본 등급이 같은 지역에 몰리면 각자 페이지의 1/n만 받는 자리였다. 나누는 방식은 **당번표**(후보 id 정렬 + 기준 공고 id 해시를 시작점으로 연달아 셋) — 페이지 id와 광고 id를 이어 붙여 해시하는 안은 djb2 뒷글자 편향으로 한 곳이 99%를 독식해 실측으로 버렸다(SNAPSHOT 2026-09-05). 먼저 산 교회 것이 묽어지는 것은 운영자가 알고 택했다
...
- [x] ✅ **이용 측정 — GA4 (2026-09-06)** — 이벤트 9개 + 맞춤 측정기준 8개(정본 `docs/ANALYTICS.md`), 핵심 지표 `source_click`(원문으로 나감 = 지원하러 감). 검색 유입은 획득 보고서의 Organic Search로 본다. Vercel env는 넣었고 내부 트래픽 제외는 안 한다(운영자 결정). ⬜ 배포 뒤 첫 클릭이 들어오면 `source_click`에 주요 이벤트 ★ · 첫 판단은 데이터 4주 뒤(ANALYTICS §5)
- [ ] 공고 100개 목표 달성
