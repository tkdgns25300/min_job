...
### 1-8. 배포 & 결제(NHN KCP) 심사 — 선행 (카드 신청·PG 심사가 오래 걸려 앞당김)
> **전략**: admin 제외 기본 페이지가 mock으로 완성되면, **실 데이터/백엔드보다 먼저** 배포하고 NHN KCP 가맹 신청·사이트 심사를 넣는다. 심사가 도는 동안(오래 걸림) 데이터·백엔드·기능을 마저 다듬는다. 노출 광고의 **온라인 결제는 KCP 승인 후 연동**한다("문의 결제"는 지금의 과도기 표현).

순서:
1. [ ] **admin 제외 기본 페이지 mock 완성** — 남은: `/terms`·`/privacy` 확정 · **취소/환불 규정** · **전자상거래법 사업자정보 표기(푸터)** · **SEO(`sitemap.ts`·`robots.ts`)**. (`/jobs/new` 인증 게이트는 적용 완료)
2. [ ] **Vercel 배포** — 지금 **JSON 더미 데이터 그대로**(심사용) · 도메인 연결
3. [ ] **NHN KCP 가맹 신청 + 사이트 심사 요청** — 심사가 보는 것(일반 PG 요건 ※정확한 요건은 KCP 신청 시 안내 확인): 상품·가격(`/pricing`) · 이용약관 · **취소/환불 규정** · **사업자정보 표기**(상호·사업자번호·대표·주소·통신판매업 신고번호 등) · 개인정보처리방침. **결제 모듈 연동(코드)은 승인 후**
4. [ ] **(승인 후/병행) 실 데이터 + Supabase 백엔드 + KCP 결제 연동 + 기능 상세 다듬기** → Phase 1 본체(1-1~1-7)와 합류
