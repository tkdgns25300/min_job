...
> **순서 확정(2026-07-20)**: 배포 → PortOne 테스트 결제 → KCP 심사 → (심사 중) admin·상세 기능·데이터.
...
5. [ ] **NHN KCP 가맹 심사(진행중)** — 기존 상점(같은 사업자)에서 가맹 심사 진행중. 심사가 보는 것(일반 PG 요건 ※정확한 요건은 KCP 안내 확인): 상품·가격(`/pricing`) · 이용약관 · **취소/환불 규정** · **사업자정보 표기**(상호·사업자번호·대표·주소·통신판매업 신고번호 등) · 개인정보처리방침. **실결제(온라인 승인)엔 일반결제 계약 필요**(현재 계약취소 → 재계약)
6. [ ] **(승인 후/병행) 실 데이터 + Supabase 백엔드 + 실결제 전환 + 기능 상세 다듬기** → Phase 1 본체(1-1~1-7)와 합류
