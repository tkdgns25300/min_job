...
- [ ] 마이그레이션 `001_init.sql` — churches·jobs·users·claims 테이블 + enum CHECK + 인덱스 + RLS
...
### 1-1. 공통 골격
- [ ] 도메인 타입 (`types/domain.ts`) — Job·Church·User·Claim
...
### 2-2. 소유권 인수 (claim) — manual seeding → 자생 운영
- [ ] claim 신청 (`/claim`) — 교회가 자기 교회 검색·신청
- [ ] admin claim 승인 → 소유권 연결
- [ ] claim 후 교회가 직접 공고 관리

### 2-3. 구직자 편의
- [ ] 구직자 로그인 + 북마크(공고 스크랩) — `bookmarks` 테이블

### 2-4. 광고·노출 (수익화 — 첫 매출 채널)
...
> **Phase 2 완료 기준**: 재공고 정보 노출 + 교회가 자기 공고를 claim해 직접 관리 가능 + 첫 프리미엄 노출.
...
- [ ] 사이트 내 지원·서류 중개
- [ ] 공고 재등록 편의 (claim한 교회의 복붙 재게시) — 유료 끌어올리기는 Phase 2 광고·노출
- [ ] 결제 연동 (프리미엄 노출 자동 과금)
...
| R3 | 교회가 무료라도 등록·claim하러 올까? | manual seeding으로 우회, 자생 전환 미검증 |
