...
- [x] ✅ **교회 인증 스키마 확정 (2026-08-18)** — **새 테이블 없이** 기존 7개로 해결. DATA §1·§3·§9·§11 반영:
  ...
  - ✅ **Server Action 2개 다 배선됐다** — `mypage/verify/actions.ts`(제출 · 2026-08-25) · `admin/verify/actions.ts`(승인·반려 · 2026-08-26). ⚠️ 증빙은 "즉시 파기"가 아니라 **반려일 때만 파기**한다(방침 개정 2026-08-25). 명세는 **SPEC 교회 인증·`/admin/verify/[id]` 절**이 정본
...
- [x] ✅ **입력 경로가 `jobs`를 다 채우지 못하던 것 — 5개 전부 닫힘 (2026-08-21 실측 → 2026-08-26~27 해소)** — 예고한 대로 **공고 등록·수정 mutation과 한 묶음으로** 닫았다.
  - **`/jobs/new`·`/jobs/[id]/edit` 폼에 입력칸이 없던 컬럼 5개**
    - [x] `job_kind` — **NOT NULL이고 CHECK ①이 `position`/`role`과 묶여 있다.** 즉 그때 폼으로는 애초에 **유효한 행을 만들 수 없었다** → "모집 종류" 단일선택 칩(2026-08-26). 고르지 않은 쪽 자리는 `common()`이 비워 CHECK 짝을 맞춘다
    - [x] `role` — 일반직 직무. `job_kind`와 한 짝 → 일반직을 고르면 직무명 입력칸이 뜬다
    - [x] `qualification` — 자격/경력 **enum**(필터축) → **2026-08-27 추가**. 안 받는 동안은 예고대로 교회 공고가 자격 필터에서 통째로 탈락했다(`filter-jobs.ts`가 값 없는 공고를 버린다). 근거: 크롤 863건 중 608건(70%)에 값이 있어 실제로 도는 축이고, **직분으로 대체되지 않는다** — 부목사 공고 135건이 신학생 74 / 목사안수 50으로 갈린다. ⚠️ 크롤러가 실제로 내보내는 값은 셋뿐이다(신학생 497·목사안수 97·경력 22) — `ANY`·`ENTRY`는 0건
    - [x] `preferred` — 우대 사항. 공고 상세엔 섹션이 있는데 교회가 채울 수 없었다 → 자격과 **다른 프리셋**으로 분리(겹치는 항목을 두지 않는다 — 한 줄이 필수와 우대에 동시에 뜨면 서로 모순된다)
    - [x] `housing_note` — 사택 비정형("사택 전세 지원 5천만원") → 칩 옆 설명 입력칸(2026-08-27). **칩 자체는 필수로 올렸다**(설명은 선택) — 근거는 SPEC. **칩과 함께** 저장돼 `housingLabel`이 "제공 · 전세 5천만원 지원"으로 붙여 그린다. ⚠️ `"협의"`는 교회가 쓴 글이 아니라 칩이 `provided=null`을 "정보 없음"과 가르려고 넣는 한 단어라 **되읽을 때 입력칸에 넣지 않는다**(`housingNoteOf`)
...
- [x] ✅ **`qualification`의 `NULL` 정의를 바로잡음 (2026-08-27)** — DATA가 `NULL`을 **"무관"** 이라 적어 뒀는데 **틀린 문장이었다.** 크롤러는 *"`무관`이라고 **적혀 있으면** `ANY` / 자격 이야기가 **없으면** `null`"* 로 갈라서 보낸다(min_job_agent `extraction.py`) — 사택의 `false`(명시적 미제공) vs `null`(정보 없음)과 **같은 구분**이다. 고친 곳: `docs/DATA.md`(2곳) · `types/domain.ts`.
  > ⚠️ **한 번 반대로 갔다가 되돌렸다 — 기록으로 남긴다.** "자격 필터에서 `무관` 칩이 0건을 돌려준다"를 보고 `ANY`가 `NULL`과 겹치는 중복 값이라 판단해 **라벨 맵에서 지웠다가**, 크롤러 리포를 열어 보고 되돌렸다. 지우면 *"무관이라고 밝힌 공고"* 와 *"아무 말 없는 공고"* 가 한 덩어리가 된다 — 원문이 **한** 말을 우리가 지우는 쪽이라, 안 한 말을 지어내는 것과 같은 무게의 실수다. 교훈: **형제 리포가 같은 컬럼을 쓰면 우리 문서만 보고 값을 지우지 않는다.**
  > ⚠️ `ANY`가 실측 0건인 것은 값이 죽어서가 아니다 — 크롤러가 겹칠 때 `ORDAINED > SEMINARIAN > EXPERIENCED > ENTRY > ANY` 순으로 고르는데 `ANY`가 **꼴찌**라 "경력 무관"은 `EXPERIENCED`로 간다. 이기기 어려운 값이지 잘못된 값이 아니다. 따라서 **"무관" 칩이 지금 0건인 것은 정상**이다(데이터가 없을 뿐 — 공고 없는 교단 필터가 0건인 것과 같다).
  > 🔴 **딸려 나온 진짜 버그 — URL 필터가 아무 문자열이나 받고 있었다.** `parseJobsUrlState`가 다중선택 축 6개를 `new Set(sp.getAll(dim))`로 **검증 없이** 담았다. 바로 위 주석이 *"잘못된 값은 기본값으로 폴백해 오래된 링크에도 깨지지 않게 한다"* 고 약속하는데 이 축들만 그 밖에 있었다. 증상: `?region=MARS` 같은 링크가 **0건을 띄우는데 눌린 칩은 없어** 왜 안 나오는지 알 수 없는 막다른 화면이 됐다. `lib/domain-enum`의 `keysOf`가 정확히 이 일을 하는 함수인데 이 파서만 안 쓰고 있었다 — 축→라벨맵 표(`DIM_OPTIONS`)를 두고 통과시킨다. 실측: `?region=MARS&position=XYZ`가 **854건 전체**로 복구, `?qualification=SEMINARIAN`은 497건(정상).
...
- [~] 교회 상세 (`/churches/[id]`) — **mock UI 완료**. 교회 정보 + 현재/지난 공고 + 지도 링크(갤러리는 2026-08-29 제거 — Phase 2). 공개는 `verification_status='APPROVED'`만(DATA §9). 교회 목록 browse는 두지 않음 — SPEC 참조
...
- [x] ✅ **`/mypage` 사역자 view 재구성 — 2026-08-28.** 초기 기획(7/12) 화면에 "준비 중" 섹션 둘(관심 교회 · 회원탈퇴 상자)이 실제 기능 셋과 같은 무게로 놓여 있었고, 인증 교회 담당자에게 유일하게 쓸 카드가 스크롤 세 번 아래였다. 고친 것: ① 교회 카드를 **인증·신청중이면 맨 위**, 구직자에겐 아래(사람에 따라 순서가 다르다 — `mypage-view.tsx` 머리말) ② 관심 교회 섹션 삭제(Phase 2 그대로) ③ 회원탈퇴 상자 → 한 줄(경로는 그대로) ④ **최근 본 공고를 저장 목록과 같은 행으로**, 서버 카드에서 id로 찾아 그려 **지워진 공고가 자동으로 빠진다**(한때 localStorage 캐시 제목을 그대로 그려 지운 공고가 눌러도 없는 페이지로 남았다) · 5개까지 · 빈 상태 한 줄. 명세는 SPEC. 실측: 저장 10건 중 지워진 3 + 테스트 2 + mock 시절 1을 걸러 4건이 떴다.
  > ⚠️ **헤더 아바타는 건드리지 않았다** — 검수 중 "인증 교회면 `/mypage/church`로 직행"을 검토했는데 로그아웃·탈퇴가 `/mypage`에만 있어 그 길이 사라진다. 아바타=계정 / "교회 공고 등록" pill=작업, 라벨도 목적도 달라 다른 곳으로 가는 게 맞다.
  > ⬜ **`/jobs` 우측 레일(`recently-viewed`)·검색 오버레이(`search-box`)는 여전히 localStorage 캐시 제목을 그린다** — 같은 결함(지운 공고가 남는다)이 그 두 곳엔 남아 있다. 마이페이지처럼 카드로 그리려면 그 화면들에 전체 카드가 없어 다른 풀이가 필요하다(id 목록만 서버에 보내 확인). ⚠️ 북마크 DB 이전(2026-08-28)에서 **의도적으로 남겼다** — 그 두 화면은 방문마다 서버를 부르지 않는 것이 지금의 장점이고, `getRecentJobCards` 액션이 이미 있으니 붙이는 건 쉽지만 그 트레이드오프를 따로 정해야 한다.
- [~] 마이페이지 (`/mypage` · `/mypage/church` · `/mypage/church/info` · `/mypage/church/promote`) — 사역자 view(최근 본 + **북마크** + 하단 교회 CTA·계정) + 교회 대시보드(상태 탭·노출광고 사이드바 · 행 액션은 **수정 하나**) + **교회 정보 관리 ✅ 실 저장(2026-08-27)** + 노출 결제 페이지(PortOne V2 실결제 · **Server Action 완료 처리 — 정원·주문 저장·적용 · 2026-09-03**, 1-8·4). 헤더 아바타=마이페이지 직행 + "교회 공고 등록" 상시 링크(`hasChurchAccess` 분기).
  > ✅ **교회 정보 저장 (2026-08-27)** — `lib/church-info.ts`(순수 규칙) + `info/actions.ts`. `churches` 6칸 + `church_links` 행을 쓰고 `updateTag("churches")`로 교회 상세에 즉시 반영. 명세·결정은 SPEC. 검수·종단 테스트에서 나온 것:
  > · 🔴 **채널 칸의 `type="url"`이 제출 자체를 막고 있었다** — "주소만 붙여 넣으면 `https://`를 붙여 준다"는 설계와 브라우저 기본 검사가 충돌해, 저장을 눌러도 `submit` 이벤트가 안 나갔다(클릭은 됨). `type="text"` + `inputMode="url"`로 바꿨다. **목업·타입검사·빌드로는 안 나오는 결함**이다.
  > · 🔴 **`javascript:`가 공개 페이지 `href`로 나갈 수 있었다** — 스킴이 **있어서** 자동 부착을 비껴가고 `new URL()`도 통과한다. `http`/`https`만 통과시키고 호스트 모양(`https://.com`·`https://a.`)과 자격증명(`https://naver.com@evil.com`)까지 막았다.
  > · 🟠 **`upsert`에 `onConflict`가 없으면 두 번째 저장부터 터진다**(기본이 PK 충돌이라 매번 새 삽입 → `UNIQUE(church_id, type)` 위반). 지우기는 "남길 것 빼고 전부"이고 **남길 게 없을 때 조건을 아예 걸지 않는** 분기가 따로 있다.
  > · 🟠 **부분 실패 문구가 절반은 거짓이었다** — 넣기는 됐고 *지우기*만 실패하면 적은 것은 다 저장된 상태다. `save`/`cleanup`으로 갈라 각자 다른 말을 한다.
  > · 거짓 주석 하나(인증 큐가 `churches.contact_*`를 보여준다 — 실제로는 **신청자가 적어낸 값**을 보여준다)와 틀린 숫자 하나(`Field` 호출부 48곳 → 실제 35곳)를 고쳤다.
  > ⛔ **사진 업로드는 MVP 범위 밖으로**(2026-08-29 결정) — 화면의 사진 구획도 함께 걷어냈다(아래 Phase 2 항목).
- [x] ✅ **성공 알림을 토스트로 통일 — 2026-08-27.** `sonner` + `components/ui/sonner.tsx`(라이트 고정·우상단·헤더 높이만큼 offset), `<Toaster />`는 **루트 레이아웃에 한 벌**(로그아웃처럼 그룹을 넘는 이동이 있어 그룹별로 두면 사라진다). 규칙 = **성공은 토스트 · 실패는 인라인**(CLAUDE.md Styling). 배선 **15지점**: `/admin` 새로고침 1 · `/admin/jobs` 목록 2 · `/admin/jobs/[id]` 3 · `/admin/review/[id]` 3 · `.../group` 2 · `/admin/verify/[id]` 2 · `/mypage/church/info` 1 · 공고 상세 링크 복사 실패 1(유일한 `toast.error` — 성공은 아이콘이 말하고 인라인 자리가 없다).
  > 🔴 **판정 화면 넷은 토스트를 띄울 수 없는 상태였다** — `redirect(QUEUE_PATH)`가 액션 안에 있었고 **`redirect`는 던진다**. 그래서 `await action()` 다음 줄이 죽은 코드였다(원래 주석도 "이 줄에 오지 않는다"고 적고 있었는데, 그 줄에 알릴 말을 넣으려 한 것이 이번 작업이었다). 네 `redirect`를 걷어내고 **이동을 호출부로** 옮겼다 — 알릴 말을 먼저 띄우고 `router.push`. 이 자리가 가장 필요한 곳이었다: **반려는 되돌릴 수 없고 증빙을 파기하는데** 큐로 돌아가면 그 줄이 사라진 것만 보였다.
  > 함께 걷은 것: `done` 상태 4개(`useState` + `role="status"` 문구) · `job-actions`의 `catch {}` 삼킴 · 어드민 실패 문구 6개의 말투(요체 → **습니다체**, 어드민 실측 67:10 · 두 개는 한 문장 안에서 섞여 있었다) · `role="alert"` 누락 2곳(`promote-checkout`·`verify-form` — 같은 파일 형제 문구엔 있었다).
  > ⛔ **부분 실패에 색을 따로 주지 않았다**(검토 후 기각) — `church-info`의 `save`/`cleanup`과 인증 승인의 부분 실패 모두 **다음에 할 일이 같다**(다시 저장/승인). 행동이 같은 것을 색으로 나누면 신호만 늘고 판단은 안 바뀐다. 어디까지 됐는지는 문구가 말한다(인라인이라 읽을 시간이 있다).
  > ⬜ **모바일 폭 실측 못 함** — 브라우저 줌 때문에 창을 줄여도 뷰포트가 1920으로 남았다. `mobileOffset`(top 4.5rem)은 모바일 헤더 `h-16`(4rem)을 넘고 토스트가 위쪽이라 하단 네비와 겹칠 수 없다 — 근거는 있고 실측만 없다.
- [x] ✅ **북마크 DB 이전 — 2026-08-28.** localStorage(`minjob:bookmarks`) → `bookmarks` 표. 새 seam `lib/queries/bookmarks.ts`(비캐시) + `mypage/actions.ts`의 `setBookmark`(목표 상태·멱등) + `components/job/bookmark-provider.tsx`(컨텍스트 · 헤더 hole이 seed). 저장 버튼 3곳·마이페이지 저장 해제가 한 소스를 본다. **비로그인은 로그인으로**(A안 · 사용자 결정). `/mypage`가 공고 885건 전부를 내리던 것이 **그 사람 것만**으로 바뀌었다. `lib/bookmarks.ts` 삭제, `toCard`는 `row-map`으로(세 seam이 쓴다). 실측: 홈 저장 → DB 1행 → `/jobs`·상세에서 채워진 상태로 보임 → 상세에서 해제 → DB 0행. 미리보기 iframe 안 저장·공유 `disabled` 확인.
  > ⚠️ **레이아웃 provider에 `useSearchParams`를 쓰면 안 된다** — 프리렌더되는 트리 전체가 dynamic hole이 되어 `/jobs` 목록이 셸에서 빠진다. 복귀 경로는 클릭 시점에 `window.location`에서 읽는다. 홈 프리렌더 HTML에 행 8·저장 버튼 8이 그대로 있는 것을 빌드 산출물로 확인했다.
  > ⛔ **대시보드 "북마크 수"는 넣지 않았다** — 남의 북마크를 세는 조회라 RLS를 켤 때 정책이 따로 필요하다(그때 함께). 죽은 배지("조회 · 북마크 집계 준비 중")만 지웠다.
  > ⬜ **비로그인 → 로그인 이동은 클릭으로 못 봤다**(계정이 하나라 로그아웃하면 OAuth를 다시 타야 한다). 액션이 `login`을 돌려주면 `loginPathWithNext(현재 경로)`로 보내는 코드 경로만 확인 — 비공개 창에서 한 번 눌러 보면 된다.
- [x] ✅ **교회 인증 (`/mypage/verify` 신청 + `/admin/verify` 판정) — 2026-08-25~26 실 배선.** 신청은 고유번호 확인 → 처음이면 `churches` 생성 / 기존이면 그 행에 붙임 + 증빙 서류를 비공개 버킷에. 판정 화면은 **서류를 실제로 띄우고**(1×/2×/3× 확대, PDF는 iframe) 신청값과 교회 행을 나란히 대조한다. 승인은 교회 행 먼저 → 연락처 이관(빈 칸만) → 사람 상태, 반려는 사유 필수 + 서류 파기. 재신청 레이스는 `verification_submitted_at`으로 막는다. ⬜ **결과 알림 메일은 아직 없다** — 주 5건을 넘으면 만든다(아래 별도 항목)
- [x] ✅ **교회 공고 등록·수정 (`/jobs/new`, `/jobs/[id]/edit`) — 2026-08-27 실 배선.** 3스텝 위저드 + **미리보기**(공개 화면 컴포넌트를 iframe 안에 그대로 그린다 · SPEC). 쓰는 표는 `jobs` 하나고 교회 값은 인증된 `churches`에서 복사한다. `church_id`는 세션에서 오고 클라이언트에게 받지 않는다(가드레일 #2 — 권한은 사람이 아니라 그 교회의 인증 관리자라는 자격에서 온다). 검수 없이 바로 `OPEN`(인증이 게이트). 편집 게이트 = `getEditableJob`(자기 교회 + `source=CHURCH`). 규칙은 `lib/job-draft.ts` 하나에 모아 폼과 액션이 **같은 답**을 쓴다
  > ⛔ **대시보드 행의 `⋯` 메뉴를 걷어냈다(2026-08-27)** — 등록·수정이 끝나면 돌아오는 화면인데, 안의 `마감`·`삭제`가 `onClick`으로 **메뉴를 닫기만** 했다(누를 수 있는데 아무 일도 안 하는 버튼 — `/admin/jobs`에서 같은 이유로 걷어낸 것들과 같은 부류). `삭제`는 애초에 **안 만들기로 한 기능**이고, `재등록`은 빈 `/jobs/new`로 보내 수정 화면의 **"다시 모집"** 과 다른 말을 하고 있었다. 분기도 `PENDING`이 있던 시절 모양이라 "검수중은 삭제만" 가지가 **도달 불가**였다(상태는 2026-08-21에 `OPEN`·`CLOSED` 둘로 줄었다). 행 액션은 `수정` 하나로 줄이고 상태 변경은 전부 수정 화면 하단 **상태 관리**가 한다(마감 ↔ 다시 모집 왕복 실동작 확인). `OverflowMenu`·`"use client"`·미사용 import도 함께 삭제.
  > 🔴 **종단 검증에서 잡은 것 — `다음`을 누르면 공고가 등록됐다(2026-08-27).** `다음 →`(step<3)과 `공고 등록`(step=3)이 **같은 자리의 `<button>`**이라 React가 DOM 노드를 재사용하고 `type`만 `button`→`submit`으로 바꾼다. 브라우저는 클릭의 activation behavior를 **이벤트 전파가 끝난 뒤** 판정하는데 discrete 이벤트라 그 사이 리렌더가 flush돼, 2단계에서 `다음`을 **한 번** 누르면 3단계로 가면서 폼이 제출됐다. 값이 다 찬 수정 화면에서는 교회가 지원 방법·마감·미리보기를 보지도 못한 채 저장·리다이렉트됐고, 등록 화면에서는 3단계가 빨간 오류를 띄운 채 열렸다. `key="next"`/`key="submit"`으로 노드 재사용을 끊어 고쳤다(제출 이벤트 0회 확인). ⚠️ 나머지 세 폼(`verify-form`·`church-info-form`·`search-box`)은 버튼이 자리를 바꾸지 않아 안전하다 — 확인함.
  > **함께 잡은 것(감사 2026-08-27 · 실데이터 863건 대조)**: 공개 상세의 **빈 "자격 요건" 구획**(77건에 이미 나가 있었다 — 형제 구획은 다 조건부인데 여기만 무조건) · 사례비 **최대만 적으면 화면에서 사라지던 것**(`formatPay`가 `note`로 떨어져 "협의") · 최소>최대 무검증 · `pay_*`가 `int4`인데 자릿수 상한이 없어 **DB 22003이 "저장하지 못했어요"로만 뜨던 것**(`MAX_PAY`) · **마감 지난 공고를 수정할 수 없던 것**(등록 기준 규칙을 수정에 그대로 걸었다 — 안 바꾼 값은 통과시킨다) · `MAX_LENGTHS` 5칸이 **선언만 되고 아무도 안 보던 것**(크롤 실데이터가 이미 넘긴다: headcount 285자·work_days 179·pay_note 174·benefit_note 216)
...
- [x] ✅ **지역·직분·부서 랜딩 라우트 (2026-09-03)** — `/jobs/region/[region]` · `/jobs/position/[position]` · `/jobs/department/[department]` **28개**(지역 18 · 직분 4 · 부서 6). 각자 H1·title·description·canonical·breadcrumb를 갖고 `generateStaticParams`로 프리렌더(`◐ PPR`, 값이 닫힌 집합이라 가능). 규칙 정본 = `lib/job-facets`(순수 · 테스트 19개). 라우트 파일 셋은 Next가 정적으로 찾아야 하는 것만 갖고 몸통은 `facet-page`가 한 벌로 든다
  - **조합은 만들지 않았다** — 지역×직분 79조합 중 24개가 5건 미만, 부서까지 넣으면 231조합 중 166개가 5건 미만(실측). 얇은 페이지 대량 생성은 키워드가 늘기는커녕 사이트 평가를 내리고 같은 검색어를 두 페이지가 나눠 갖는다. ⬜ **조합은 서치 콘솔에 수요가 관측된 뒤** 승격한다(건수는 공급이고 근거는 수요다)
  - 제외한 값: 직분·부서 **"기타"**(검색어가 아니다 — 자동완성이 이미 `ETC`를 빼는 것과 같은 기준) · 부서 **행정**(7건). 담기는 공고는 **사역직만**(일반직 39건은 H1의 "사역자"와 어긋난다 — 판정은 `position` 유무)
  - 얇은 축(`FACET_INDEX_MIN` 10건 미만 · 세종 3 · 제주 6 · 울산 8)은 **`noindex` + sitemap 제외**, 페이지는 살려 둔다
  - 발견 경로: sitemap + **공고 상세의 "같은 조건 모아보기"** + `/jobs` 하단 허브 + 홈 추천 검색어 칩(랜딩이 있는 말은 검색 쿼리 대신 랜딩으로)
  - ⚠️ **일반직 공고는 랜딩 링크도 breadcrumb 부모도 받지 않는다** — 랜딩이 사역직만 담으므로, 링크를 주면 담지도 않는 목록으로 보내게 된다(검수에서 잡음). 판정은 필터와 같은 술어 하나
  - ⚠️ `/jobs`의 canonical 전제는 **재검토 결과 그대로 유지**한다 — 서버 필터링은 랜딩 안에서만 하고 필터 쿼리는 계속 `/jobs`로 흡수된다
- [x] ✅ **저비용 보강 3종 완료**(2026-08-19 확인) — `BreadcrumbList`(공고·교회 상세 양쪽) · `Organization`(`(public)/layout.tsx`에서 공개 전 페이지) · `JobPosting.identifier`

### 1-6. 신뢰·법적 페이지
- [x] 소개 (`/about`) — 정적. ✅ **재작성 2026-08-31~09-01(8→6섹션)** — "공고는 이렇게 모입니다"(두 경로 · **직접 등록 먼저** — 지향점) + 교회 담당자 카드(**인증 CTA만** — 수정·삭제 안내는 문의 페이지로 미룸, 운영자 2026-09-01) 신설, "이런 분들께"·독립 문의 섹션·혜택 ③(재공고 이력 — 보류라 거짓)·FAQ "내리고 싶어요" 삭제, "조건"→"자리", 현황은 **공고→지역→교회 3수치**(교단 제외). **top nav 승격**(header `NAV_LINKS`에 소개 추가). 크롤링·자동수집 단어 미노출 프레임 유지(구성 정본 = SPEC 소개 절)
...
- [x] 문의 — 푸터 mailto (`contactMailto()`, `footer.tsx`). ⚠️ 주소는 아직 개인 메일 — 브랜드 이메일 개설 시 `constants/business.ts`만 바꾸면 된다. 푸터 공개 연락처는 전화 대신 **이메일 하나로 통일**(운영자 결정 2026-08-31 — 전화는 약관 15조·개인정보처리방침 표기에만 남는다)
- [ ] **문의 접수 페이지/폼 (운영자 요청 2026-08-31)** — 지금은 전 화면이 `contactMailto()` mailto 링크뿐이라 **메일 앱이 없는 환경(PC 웹메일 사용자)은 문의할 길이 막힌다.** 이메일로 받는 전제는 유지하되, 폼(이름·연락처·내용)을 받아 이메일로 전달하는 페이지를 만든다. 방식 후보: ① 폼 → 메일 발송(Resend 등 — 브랜드 이메일 개설과 묶으면 자연스럽다) ② 폼 → DB 저장 + 운영자 홈 노출. 교회의 공고 수정·삭제 요청(opt-out)도 이 경로가 받는다 — 소개 페이지·약관의 문의 안내와 연결
...
- [x] 사례비 — "교회 내규에 따름/면접 후 협의" UX. **등록 쪽 완료**: 폼이 프리셋 칩 + 자유 입력을 숫자(min·max)와 **동급 경로**로 두고(`job-form.tsx` `PayFields`), 표시도 `formatPay`가 `note`를 1급으로 처리한다. ✅ **표시 쪽도 완료(2026-08-29)** — 카드는 `formatPayShort`로 **금액 아니면 "협의"** 둘만 보인다(SPEC `/jobs` 5번 · 2026-08-30 운영자 결정 — 라벨 둘·말줄임 안을 거쳐 이걸로). 상세는 원문 그대로. ⚠️ 표시 규칙일 뿐 DB는 그대로다
...
**디자인 단계에서 반영**
- [x] **공고 상세 다듬기(2026-08-30 · 운영자 "더럽다" 지적)** — 실데이터(모집중 953건)로 잡은 넷: ① 지도 **플레이스홀더 상자(160px) → 주소 옆 "지도에서 보기" 링크**(교회 상세도 — 주소 있는 84%에 빈 상자가 떴다) ② 사택 "제공 · 사택제공"(102건) → 판정 본문 + 원문 표현 **보조 줄**(`housingLabel`→`housingDisplay`, `ConditionRow`에 `note`) ③ **"기타"뿐인 직분 조각 생략**(229건 · `publicPositionLabel` — 카드·SEO 제목·교회 상세 지난 공고까지, 검수·편집 화면은 그대로) ④ 안내 문장 전부 삭제 — 지원 방법 머리(두 문장 → 한 문장 → 없음)와 하단 "운영자 등록 공고예요. 잘못된 정보가 있으면 문의해 주세요."(`SourceNote`). 연락처 자체가 "교회에 직접"이고, 출처 표기는 우측 카드 원문 링크(가드레일 #1)·문의는 푸터가 맡는다 ⑤ **비슷한 공고를 공고와 갈라놓음** — 본문 아래 페이지 유일의 구분선 + 큰 여백, 제목 `text-lg` + "더 보기 →"를 제목 줄 끝으로(본문 아홉 번째 구획처럼 읽혔다) ⑥ **제출 서류를 " · " 한 줄에서 대시 목록으로**(`DocList`) — 다섯 목록 컬럼 중 유일한 예외였고, 항목 3개 중 1개가 문장이라 구분점이 묻혔다(249건이 두 줄로 넘침). 정규화는 안 한다(SPEC 1번 — 표시 규칙으로 풀고 크롤러 필드가 생기면 얹는다). **출근·제출 서류 줄은 항상 표시 유지** — 운영자 결정("어떤 공고든 늘 같은 형태") · 우측 카드·연락처 비링크도 그대로
- [x] **화면 전수 점검(2026-08-30 · 390px × 1280px · 실데이터 극단값 · 공개 11 + 교회 5 + 운영자 5 화면)** — 창 크기 조절이 막혀 iframe 하네스로 봤다. 고친 셋: ① **홈 390 오른쪽 넘침** — 2단 그리드가 `lg:grid-cols-[…]`만 있고 기본 열이 없어 암묵 `auto` 트랙이 내용 최소 폭으로 늘어났다 → `grid-cols-1`(같은 패턴 6곳 전부: 홈·`/jobs` 2·상세 2·교회 대시보드) ② **카드 메타 줄**(지역·교회·교단)이 좁은 폭에서 세 조각이 눌려 글자가 세로로 쌓임 → `flex-wrap` + 조각마다 `whitespace-nowrap`, 점은 뒤 조각에(`job-row`·`job-card`·`featured-job-card`) ③ **`/about`·`/pricing` 통계 "교회 1·지역 1·교단 1"** — `getCoverageStats`가 인증 교회 표를 셌다 → 공고 기준(홈과 같은 키)으로, 921·794·18·10. 판단만 남긴 것: 390 상세 카드의 174자 사례비(원문 그대로 결정) · 요금 비교표 가로 스크롤 힌트 없음 · 요금 히어로 "온라인 결제는 준비 중" 문구(BM 재기획 ③에서). 글자 크기 인벤토리: 표준 9단계 + 임의 px 85곳(18종) — 10px 이하 15곳은 전부 요금 미니 목업(`exposure-scenes.tsx`), 그 밖엔 없음. **2차(같은 날)**: 같은 출처 하네스(`public/__audit.html` 임시, 커밋 안 함)로 **320·360·768·1024·1920 × 22화면 가로 넘침을 스크립트로 전수 측정 — 0건**, 768·360은 눈으로도 확인. 시트 둘(필터·모바일 메뉴)도 열어 봤다 → 함께 잡은 것 ④ **모바일 메뉴 `SheetClose render={<Link>}`가 매 페이지 콘솔 에러**(Base UI "native button 기대") → `nativeButton={false}` ⑤ **필터 시트 안 "상세 필터" 제목 중복**(SheetTitle + JobFilter 머리) → SheetTitle을 `sr-only`로, 그러자 시트의 닫기 X가 JobFilter 머리의 "초기화"와 한 줄에 겹쳐 `pt-12`로 머리를 X 아래로. ⚠️ 도구 메모: MCP 탭은 백그라운드라 rAF가 멈춰 Base UI 시트가 `data-starting-style`에 걸려 투명하게 보인다 — 스크린샷을 한 번 찍어 프레임을 강제하면 정상. 버그로 착각하지 말 것. 미확인: 요금 미리보기 모달 · 실기기(iOS Safari)
...
- [ ] ⏸ **교회 사진 업로드 → 교회 상세 갤러리 — Phase 2로(2026-08-29 결정 · MVP 범위 밖).** 교회 직접 등록 시만(운영자 수집분은 저작권 이슈로 제외, 포스터 이미지와 동일 논리).
  > **코드는 다 뺐다** — 정보 관리의 사진 구획(`disabled` 버튼 + "준비 중"), `ChurchGallery`(라이트박스), 교회 상세의 갤러리 분기, `Church.photos`, `row-map`의 `church_photos` 조인. 데이터가 영원히 0장인데 갤러리 청크가 방문자 전원에게 나가고, `disabled` 버튼은 고장으로 읽혔다. **표 `church_photos`는 남겼다**(마이그레이션 왕복을 피한다). 갤러리 원본은 **`e586fe0`** `components/church/church-gallery.tsx`.
  > **되살릴 때의 설계(2026-08-29 검토 완료)**: 공개 버킷 `church-photos`(5MB · jpeg/png/webp) · 쓰기는 증빙 서류처럼 Server Action + `service.ts`(Storage RLS 정책은 만들지 않는다) · **브라우저에서 긴 변 1600px JPEG로 리사이즈 후 업로드**(폰 사진 5~12MB > 액션 바디 상한 4.5MB, HEIC도 이 과정에서 풀린다) · 경로 `{church.id}/{uuid}.jpg` `upsert:false` · 최대 6장 · 첫 장=커버 + "대표로" 버튼(드래그 정렬 X) · 삭제는 DB 먼저 파일 나중 · 저장 버튼과 무관하게 즉시 반영 · `updateTag("churches")`.
...
- [x] ✅ **디버깅 묶음 (운영자 리스트 7건 · 2026-08-29)** — ① `/jobs` 페이지네이션 윈도우(47개 전부 나열 → 현재 ±1·첫·끝·줄임표·이전/다음 → **2026-08-30 게시판식 `« ‹ 1 2 3 4 5 › »` + `8 / 46`으로 다시** — 줄임표가 번호 사이를 끊어 읽기 나빴다, 운영자 판단. ‹ ›는 묶음 경계 넘김, « »는 처음·끝) ② 검수 `고치기`: 코드상 동작했지만 회색 글자가 버튼으로 읽히지 않았다 → 테두리 버튼 + **값을 눌러도 펼쳐짐** ③ 묶음 판정의 모든 멤버에 **검수 화면 ↗**(한때 공개된 건만 링크) ④ 검색이 통짜 substring이라 두 단어("방주교회 전임교역자")가 0건 → **단어 AND** + 플레이스홀더에 "공고 제목" ⑤ 중복 처리 메모 선택(비면 “묶음 판정 — 같은 자리”) ⑥ 운영자 홈 실패 게시판을 **링크**(`boardUrl` = 크롤러 `sources.json` 사본) + 에러 문구 `title` ⑦ 교회 직접 등록 공고의 상세에 원문 링크 대신 **배지**.
> ✅ **완료(2026-07-29)**: 위 B 구멍 마감. 겸해결=pageSize seed. ~~보류=pagination ellipsis(mock 소량)~~ → 2026-08-29 완료(아래 디버깅 묶음 ①). 회원가입/비밀번호 찾기는 **Google OAuth 전환으로 해소**(별도 가입·비밀번호 개념 자체가 없어져 "준비 중" 안내도 제거).
...
- [ ] ⚠️ **전수 조회 비용 측정** — 홈 한 번에 `getHomeFeed`·`getJobStats`·`getSearchSuggestions`가 각각 열린 공고를 **전부** 훑는다(3천 건이면 장 3개씩). 하루 캐시라 지금은 무해하지만, 줄이려면 **공개 판정을 SQL로 내리는 것**이 유일한 길이고 그건 `isPubliclyOpen` 사본을 셋으로 만드는 결정이다(지금은 거부). 실측 후 "SQL 사본 + 두 결과를 대조하는 테스트"를 둘지 판단한다
- [ ] ⬜ **시·군·구 select**(자유입력 → 닫힌 목록) — **지역 랜딩 라우트를 만들 때 함께 한다.** 지금 넣지 않는 이유: 이 폼을 쓰는 교회가 0곳인데 행정구역은 자주 바뀌고(2026년에만 인천 개편·화성 일반구·**전남광주 통합** 3건), 목록이 낡으면 그 지역 교회가 **자기 위치를 못 고른다** — 자유입력보다 나쁘다. 랜딩 라우트는 닫힌 키가 URL에 박혀야 해서 그때는 목록이 필수고, 크롤 데이터 정규화도 같이 해야 한다.
  - **목록 재생성 절차**(2분): `POST https://www.code.go.kr/etc/codeFullDown.do` + `codeseId=법정동코드` → `법정동코드 전체자료.zip`(cp949 txt, 5만여 행). `폐지여부='존재'`만 남기고 **뒤 5자리가 0**인 행이 시군구 레벨, **이름이 한 단어**인 것이 기초자치단체다(229개 · 일반구까지 세면 255개). ⚠️ 원본에 **뒤쪽 공백**이 있어 `strip()`이 필요하다(실측).
  - **크롤 실데이터 대조 결과(2026-08-26 · 777건)**: 정본과 **96.9% 일치**. 어긋난 24건은 ① 일반구 신설·시 이름만 적힘(14) ② 개편 전 옛 이름(인천 서구·중구 5) ③ 구조화 오류(`대전동구`·`광역시 서구`·`청주시 오송읍`·`고성군 고성읍` 5)뿐이다.
  - ⚠️ **`REGIONS`는 행정구역 목록이 아니라 구직 지역 축이다.** [2026-07-01 광주·전남 통합](https://www.khan.co.kr/article/202607010351001)으로 `광주광역시`·`전라남도`가 폐지됐지만 **의도적으로 따르지 않는다** — 구직자는 여전히 "광주 전도사 청빙"으로 검색하고(검색 유입이 성장 엔진), 통합 명칭으로 바꾸면 862건 마이그레이션 + 필터 URL 변경이 붙는다. **버그로 보고 고치지 말 것.**
...
- [x] ~~`/mypage` 북마크 서버 이전~~ → ✅ 2026-08-28 위 북마크 항목으로 해소. 전 공고 전달이 사라졌다.
...
- [x] **교회 인증 신청 접수 (2026-08-25)** — 화면만 있던 `/mypage/verify`에 실 배선. **판정은 운영자가 DB에서 직접** 하므로 승인·반려 액션은 만들지 않았다.
  ...
  - ⬜ 남은 것: 주소 검색(카카오) · 탈퇴 시 서류 파기(탈퇴 기능 자체가 없다) · 신청 이력 테이블 · **판정자 기록**(`users`에 컬럼이 없어 누가 승인·반려했는지 남지 않는다 — 수집 검수는 `reviewed_by`에 운영자 이메일을 찍는다. 되돌리기 없는 일방향 판정이라 더 값이 있다).
  - ⏸ **결과 알림 메일 — 주 5건을 넘으면 만든다**(2026-08-26 결정). 그때까지는 **운영자가 직접 보낸다**: 판정 화면에 신청자 이메일이 있고, 물량이 주당 몇 건이면 손으로 쓰는 편이 자동화보다 빠르고 상황에 맞는 문구를 쓸 수 있다. 지금 만들면 코드보다 **도메인 DNS(SPF·DKIM)** 설정·전달률 확인이 병목이고, 안 하면 스팸함으로 간다.
    - ⚠️ **폼 문구는 거짓이 아니다** — 신청서의 "결과를 이 주소로 알려드려요"는 *자동으로 메일이 간다*는 말이 아니다. 직접 보내면 지켜진다. **안 보내기 시작하면 그때 거짓이 되므로** 문구를 함께 고쳐야 한다.
    - ⚠️ **반려는 반드시 알린다.** 승인은 안 알려도 공고를 올리려고 다시 들어오지만, **반려는 모르면 영영 돌아오지 않는다**(승인 대기로 오해하고 기다린다) — 교회 하나를 잃는다. 손으로 커버할 때 빠뜨리면 안 되는 쪽은 이쪽이다.
    - **넘겼는지 세는 법**: `/admin/verify`의 `전체` 탭 건수 증가분. 주 5건이면 손으로 보내다 빠뜨리기 시작한다는 판단이고, 그 시점엔 Resend 무료 구간(3,000통/월)으로 충분하다.
- [x] **교회 인증 판정 (2026-08-26)** — `/admin/verify`의 죽은 승인·반려 버튼을 실제로 배선했다. **하루 전 "판정은 DB에서 수기로"를 뒤집은 것**이고, 뒤집은 이유는 그 말의 뜻이 *"사람이 판단한다"*였는데 구현이 *"사람이 SQL을 친다"*가 돼 있었기 때문이다. 판단은 사람이, 실행은 코드가 한다 — 손 SQL보다 안전하다(사유 없는 반려·한쪽 테이블만 바꾸기를 코드가 막는다). 규칙은 SPEC `/admin/verify/[id]` 절이 정본.
  - **시트 → 2열 페이지.** 증빙 서류를 화면에 띄우기로 하니 448px에서는 고유번호 10자리가 안 읽혔다. 수집 검수가 같은 이유로 2열이 됐고 그 실측을 근거로 삼았다.
  - **HEIC·HEIF를 받는 형식에서 뺐다** — 브라우저가 렌더링하지 못해 검수 화면에 못 띄운다. 화면 문구도 `DOC_FORMATS_LABEL`로 파생하게 고쳤다(손으로 적어서 HEIC가 남아 있었다).
  - **승인**은 교회 → 사람 순으로 두 테이블을 바꾸고 사무용 연락처를 **비어 있을 때만** 옮긴 뒤 `updateTag("churches")`. **반려**는 사유를 남기고 **증빙을 파기**한다(승인분은 자격이 유지되는 동안 보관 · `/privacy` §3).
  - ⚠️ **`PosterView`를 재사용하지 않았다** — 두 번째 쓰임이고("추상화는 3번째에") 필요가 다르다(서류 한 장 · 문자열이 "포스터"로 박힘 · 타입이 `queries/review.ts`에 있어 수집 검수 쿼리 모듈에 의존하게 된다). 세 번째가 생기면 `components/admin/`으로 올린다.
  - ⚠️ **`REJECTION_REASON_MAX`가 `lib/church-verification.ts`에 있는 이유** — `"use server"` 파일은 async 함수만 내보낼 수 있다(빌드는 통과하고 요청에서 터진다). 8월 25일에 겪고 또 겪었다.
  - ⚠️ **`.select()`에 문자열을 `+`로 이어 붙이면 안 된다** — 타입이 `string`으로 넓어져 PostgREST 행 추론이 죽고 결과가 `GenericStringError`가 된다(실측).
  - 검증: 브라우저에서 **신청 → 반려 → 재신청 → 승인** 전 구간을 실제로 눌러 확인했다. 반려 시 사유 저장 + **Storage 서류 0개**(파기 확인) · 재신청이 기존 교회 분기를 타는 것 · 승인 시 두 테이블 반영 + 연락처 이관 + 탭 배지 이동 + **교회 상세가 공개됨**(`updateTag`) · 확대 1→2배 · 체크 3/3에 경고 사라짐 · 사유 없으면 반려 잠김.
...
- [x] ✅ **반대 방향(더 흔할 것) — 등록 시 클레임 유도. 구현 2026-09-01** — `/jobs/new` 등록 전 후보 패널(`claim-panel`) + `claimJob` 액션. 규칙 정본 = `claimMatchTier`(lib/job-church — 이름 정규화 일치/포함 + 지역·교단이 둘 다 명시인데 다르면 제외 · **확정은 교회가**, 자동 병합·자동 마감 없음). 액션은 서버에서 규칙을 재판정하고 `church_id IS NULL` 재확인으로 동시 클레임 경합을 막는다. `church_id`+`source=CHURCH`로 이전(원문 링크·게시일 보존 — 가드레일 #1 출처 표기, 상세 "직접 등록" 배지는 원문 링크 없는 공고로 한정). 대시보드 클레임 배너·`claimableCount`는 걷어냈다(입구는 등록 화면 한 곳 — 운영자). E2E: 임시 크롤 공고 심어 후보 노출→가져오기→수정 화면→관리 목록까지 확인 후 삭제. (원래 계획 메모) 크롤링 공고가 먼저 올라가 있고(`source=OPERATOR`) 그 교회가 나중에 인증해 직접 올리려는 경우, 새로 등록하면 중복이 된다. **운영자가 개입할 지점이 없다**(교회가 등록 버튼을 누르면 끝). → **`/jobs/new` 제출 직전에 그 교회의 `source=OPERATOR` 공고를 보여주고 "혹시 이 공고인가요?"** — [네, 가져와서 수정] / [아니요, 새로 등록]. 중복을 막으면서 **클레임이 자연스럽게 일어난다**(지금 클레임은 대시보드에 묻혀 있어 교회가 잘 못 찾는다).
  > 🔗 **편집 게이트와 짝**: `getEditableJob`이 `source=CHURCH`만 편집 허용하므로(2026-08-07), 교회가 운영자 공고를 수정하려면 **반드시 클레임을 거친다**. 위 유도 화면이 그 유일한 입구다.
  > ✅ **`housing_note` 유실 위험은 미리 없앴다(2026-08-27).** 그전까지 등록 폼은 사택을 칩 3개로만 받고 저장된 원문을 무조건 `"협의"`로 덮었다 — 크롤 863건 중 **355건이 "협의"가 아닌 원문**이고 그중 **69건은 금액이 적혀 있다**("사택 전세 지원 5천만원"). `source=CHURCH` 게이트 덕에 닿지 않았을 뿐이라 **클레임이 열리는 순간 터질 자리**였다. 칩 옆에 설명 입력칸을 두어 원문을 읽고 그대로 되쓰게 했다(7가지 모양 왕복 검증). 부분 UPDATE(131줄)로는 못 풀 문제였다 — 폼이 실제로 그 칸을 쓰기 때문이다.
...
- [~] **UI 문체 규칙 확정 후 일괄** — **세 구간이다**(2026-08-27 실측으로 어드민 구간 확정): **어드민=습니다체**(실측 67:10 — 토스트·액션 실패 문구는 이때 맞췄다) / **교회·구직자 화면=해요체** / **약관·개인정보=합니다체**. ⚠️ 한때 "제품 화면=해요체"로 적었는데 그러면 어드민이 뒤집힌다.
  > ⬜ 남은 혼용: 어드민 **빈 상태 문구·툴팁 9곳**(`review-queue-view` 3·`verify/[id]/doc-view` 2·`admin-verify-view` 1·`admin-jobs-view` 1·`job-row`/`job-edit-form`의 `title` 2) — 이 중 **둘은 한 문장 안에서 섞인다**(`doc-view:32` "파기했어요 … 올라옵니다", `review-queue-view:186` "아니에요 — … 있습니다"). 그리고 `pricing/page.tsx`(한 답변에 두 문체). 이번엔 **액션이 돌려주는 시스템 문구만** 손댔다 — 빈 상태·힌트는 성질이 달라 함께 정한다.
...
> 일부 기능은 DATA.md의 B 결정(거리·유료 노출 만료)에 의존.
...
### 2-2. 사역자 편의 (재방문 유도)
- [ ] **관심 교회 팔로우 + 새 공고 알림** (`church_follows`). (로그인·북마크는 단일 계정이라 Phase 1로 이동 = 1-4) ⚠️ `/mypage`의 "준비 중" 자리는 2026-08-28에 뺐다 — 만들 때 섹션을 다시 넣는다(알림 채널 = 브랜드 이메일 선행).
...
- [x] ✅ **BM·노출 상품 재기획 — 확정(2026-09-02)** — 자리 셋(홈 카드 3칸 · 목록 상단 로우 최대 5줄 · 연관 첫 칸) + **사다리 3등급**(스페셜 9.9만/주·정원 3 → 플러스 4.9만·정원 2 → 기본 2.9만·정원 없음, 1·2·4주, VAT 포함) · 광고는 그 화면의 결과에 원래 들어갈 공고일 때만 맨 위(홈 항상 / 목록 필터 통과 / 연관 자격 문 + 같은 지역) · 정렬 1차 키 폐기 · 라벨은 섹션당 "광고" 하나 · **런칭 프로모션 없음** · 성과형(클릭·지원당)은 측정 0·트래픽 0이라 제외 · **비슷한 공고 규칙 교체**(자격 문 → 선호 점수 → 보충). 정본 = SPEC 수익화·공고 상세 절, 근거 = SNAPSHOT §9. **코드는 아직 2단** — 아래 체크리스트를 결제 마무리(1-8)와 한 묶음으로.
  > 재기획 착수 문구(2026-08-30): 지금 상품(프리미엄·대표광고 2종, `EXPOSURE_PRODUCTS` 4개 가격)은 **2026-07-08 코드 전 초기 계획**이다. 실배포 전이라 **어떤 상품으로 갈지**를 다시 정한다: 종류(2종 유지? 끌어올리기·묶음?) · 노출 위치(홈 히어로·목록 상단·카드 강조) · 기간 단위(1·2·4주?) · 가격·묶음 할인·부가세 표기 · 결제 수단. 근거 = 실데이터(모집중 953건 · 교회 인증 0곳 · 크롤 공고가 대부분이라 유료 노출의 경쟁 상대는 무료 크롤 공고다) + SNAPSHOT §9 조사. **모델(무료 공고 + 유료 노출)은 재론하지 않는다.** 결과는 `EXPOSURE_PRODUCTS`·`/pricing`·`/mypage/church/promote` 셋에 반영되고, **결제 마무리(1-8 주문 저장·노출 적용)는 이 뒤에** 붙인다
  > 이전 문구(2026-07): 상품은 프리미엄·대표광고 2종으로 진행(끌어올리기는 저볼륨이라 제외, 기독 B2B 배너는 Phase 2+ 옵션). 상세(노출 위치·기간 단위·가격·묶음 할인·부가세·결제 수단)와 최종 확정을 여기서 검토 — 이게 돼야 `/pricing` 실제화 가능. → 재기획의 출발점으로 남긴다
...
- [ ] ⏳ **운영자 임시 결제 가격 제거** — 실결제 점검을 위해 `ADMIN_EMAILS` 계정에만 정액(스페셜 300·플러스 200·기본 100)을 적용해 뒀다(2026-09-03). 요금 페이지·다른 교회는 정가를 본다. **점검이 끝나면 지운다** — 세 곳뿐이다: `constants/domain.ts`의 `TEST_EXPOSURE_PRICES`와 `exposurePrice` 셋째 인자 · `promote/page.tsx`의 `testPricing` prop · `promote/actions.ts`의 `isOperatorEmail` 인자. 남겨 두면 운영자 계정이 계속 100원에 광고를 산다
