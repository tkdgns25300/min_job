...
> **순서 확정(2026-07-20)**: 배포 → PortOne 테스트 결제 → 도메인 연결 → KCP 전자결제 신청(심사) → (심사 중) admin·상세 기능·데이터.
...
7. [ ] **(승인 후) 실 채널 전환** — PortOne 실연동 채널(KCP 일반결제) 키를 `NEXT_PUBLIC_PORTONE_CHANNEL_KEY`(로컬·Vercel)에 교체 + 일반결제 계약 활성 확인. 기존 "Cinelab NHN KCP" 채널은 옛 서비스 것일 수 있어 확인/신규 생성
8. [ ] **(승인 후/병행) 실 데이터 + Supabase 백엔드 + 기능 상세 다듬기** → Phase 1 본체(1-1~1-7)와 합류
