...
- [ ] 교회 공고 등록·수정 폼 (`/jobs/new`, `/jobs/[id]/edit`) — '교회 직접 등록', 본인 소유. 교단은 enum 드롭다운(자유입력 금지)
...
- [x] 공고 노출 안내 (`/pricing`) — 정적, 가격 "문의" (실제 광고 기능·가격은 Phase 2)
...
- [ ] 교회 규모(대/중/소) **제거** — 기준 모호·신뢰 저하·"세상적". domain·카드·필터·상세·DATA에서 삭제. 규모 감(感)은 지도·건물사진으로 대체
- [ ] 직분에 **담임목사 추가 여부** ⚠️**결정 필요** — 실수요 있으나 "부교역자(부목사·전도사)" 포지셔닝과 상충
...
**홈·정보구조**
- [ ] 홈 스탯 **"청빙 중 교회" 제거** → 모집중·새공고 2개 (한 교회 여러 자리도 한 게시글로 인식돼 혼란)
