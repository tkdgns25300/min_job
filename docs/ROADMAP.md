...
- [x] ✅ **자격 필터의 "무관"이 0건을 돌려주던 것 (2026-08-27 발견·해소)** — DATA §2는 `qualification`의 `NULL`을 이미 **"무관"으로 정의**하는데 enum에 `ANY`가 또 있어 **같은 뜻의 철자가 둘**이다. 크롤러는 `ANY`를 쓰지 않고(실측 887건 중 **0건**), `filter-jobs.ts:54`가 값 없는 공고를 버리므로 → **구직자가 "무관" 칩을 고르면 887건 중 0건**이 나온다. 정작 무관에 해당하는 `NULL` 263건이 통째로 빠진다.
  > **①을 택했다 — `ANY`를 라벨 맵에서 뺐다.** DATA 정의(`NULL`=무관)와 일치하고, 칩을 없애도 잃는 것이 없다(자격을 안 따지는 구직자는 필터를 걸지 않으면 전부 본다).
  > ⛔ **②(필터에서 `ANY`가 `NULL`도 잡게 하기)를 택하지 않은 이유**: "안 적었을 뿐"인 공고를 "조건 없음"으로 **단정**하게 된다. `housingLabel`이 `null`(정보 없음)과 `false`(명시적 미제공)를 가르는 것과 같은 이유다 — 원문이 하지 않은 말을 우리가 하게 된다.
  > ⬜ DB CHECK에는 `'ANY'`가 남아 있다(0건이라 마이그레이션은 미룸). 크롤러가 보내도 `keyOf`가 `NULL`로 좁히는데 `ANY ≡ NULL`이라 뜻이 보존된다 — **크롤러 쪽 enum에서도 뺄지는 그쪽 리포와 맞출 것**.
  > 🔴 **딸려 나온 것 — URL 필터가 아무 문자열이나 받고 있었다.** `parseJobsUrlState`가 다중선택 축 6개를 `new Set(sp.getAll(dim))`로 **검증 없이** 담았다. 바로 위 주석이 *"잘못된 값은 기본값으로 폴백해 오래된 링크에도 깨지지 않게 한다"* 고 약속하는데 이 축들만 그 밖에 있었다. 증상: `?region=MARS`·`?qualification=ANY`(방금 없앤 값) 같은 링크가 **0건을 띄우는데 눌린 칩은 없어** 왜 안 나오는지 알 수 없는 막다른 화면이 됐다. `lib/domain-enum`의 `keysOf`가 정확히 이 일을 하는 함수인데 이 파서만 안 쓰고 있었다 — 축→라벨맵 표(`DIM_OPTIONS`)를 두고 통과시킨다. 실측: `?qualification=ANY`·`?region=MARS&position=XYZ` 둘 다 **854건 전체**로 복구, `?qualification=SEMINARIAN`은 497건(정상).
  > 같은 뿌리의 관찰: `ENTRY`("초임 가능")도 크롤 0건이다 — 이 다섯은 우리가 만든 축이고 교단 게시판은 경력을 **연수로** 쓴다(`constants/domain.ts`의 `QUALIFICATIONS`).
