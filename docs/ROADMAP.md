...
- [ ] 로그인 (`/login`) — Supabase Auth, 간편 로그인(카카오·네이버·구글) + 이메일. **단일 계정 = 기본 사역자**(가입 시 역할 선택 없음)
- [ ] 마이페이지 (`/mypage`) — **사역자 view**(최근 본 + **북마크**) + **교회 view**(인증 시 공고 관리) + 두 view 전환
- [ ] **북마크** (`bookmarks` 테이블) + 공고 카드·상세 저장 버튼 — 단일 계정이라 **Phase 1로 이동**(원래 Phase 2). 지금은 localStorage로 동작
- [ ] 교회 인증 (`/mypage/verify`) — 교회 선택/생성 + 증빙(고유번호증/사업자등록증) → 운영자 승인 → 인증 교회만 게재
- [ ] 교회 공고 등록·수정 (`/jobs/new`, `/jobs/[id]/edit`) — '교회 직접 등록'. **인증 게이트** + 편집 권한 = 교회 인증 멤버십(owner 아님). 교단 enum 드롭다운(자유입력 금지)
