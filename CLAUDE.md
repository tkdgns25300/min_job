...
│   │   │   │                      모바일 복귀 `?paymentId=`) · promote-checkout(등급·시작일·기간) · promote-outcome ·
│   │   │   │                      promote-complete · actions.ts(결제 완료 `completePromotion` — 원장에 한 줄 = 노출))
...
│   │                              actions.ts(등록·수정·마감·클레임) + new/(+ claim-panel — 등록 전
│   │                              "이미 올라온 우리 교회 공고" 후보, 규칙=lib/job-church claimMatchTier) ·
│   │                              [id]/edit/(+ status-panel)
...
│   ├── icon.svg · favicon.ico · apple-icon.png        브랜드 마크(딥그린 타일 + 골드 M) — Next 파일 규약이라
│   │                              link 태그는 자동 주입. 세 파일 다 `scripts/make-icons.py`가 만든다(손으로 고치지 않는다)
...
│   ├── queries/promotions.ts      노출 원장 — 표시용 읽기는 `'use cache'`, 판매 판정(정원)은 캐시 금지(그 파일 머리말)
...
│   ├── job-visibility.ts          공개 만료 판정 단일 소스(todayInSeoul·addDays·isPubliclyOpen·hiddenReason)
...
scripts/make-icons.py              파비콘·앱 아이콘 만들기 — 표준 라이브러리만 쓴다(로컬에 SVG 래스터라이저가 없다).
                                   M은 폴리곤 좌표라 폰트를 타지 않는다 — 파비콘이 만드는 기계·보는 브라우저에 따라 달라지면 안 된다
```
...
- **행 → 도메인 변환은 `row-map.ts`가 한다.** enum 컬럼은 `text + CHECK`라 생성 타입이 `string`이므로 `keyOf`/`keysOf`(lib/domain-enum)로 좁힌다. ⚠️ 좁히기 실패의 기본값은 **덜 보이는 쪽**이다(`status`→`CLOSED`, 원장 `tier`는 **행째 버린다**) — 모르는 값을 공개·유료 노출로 읽으면 사고가 된다.
...
- **예외(인증 의존·PII read)** — `'use cache'`/`cacheTag`를 쓰지 않는 함수들:
  - `users.ts` **전체**(`getCurrentUser`·`getChurchDashboard`·`getEditableJob`·`getClaimCandidates`) — 모두 로그인 사용자(또는 그 인증 교회)에 종속돼 방문자마다 결과가 다르다. `getClaimCandidates`는 **가져간 즉시 목록에서 빠져야** 해서도 캐시하지 않는다. `getCurrentUser`는 `server.ts`(쿠키 세션)를 쓰고 `React.cache`로 요청당 1회만 왕복한다 — 신원은 `auth.users`(Auth API), 소속·인증상태는 **`public.users` + `churches` 조인**에서 온다(`auth` 스키마는 PostgREST JOIN이 안 돼 프로필을 복제해 둔다).
  ...
  - `promotions.ts` — **노출 구매 원장**(`job_promotions`)의 **판매 판정**(정원·경합·겹침)과 운영자 원장 표. 정원("9월 7일 스페셜이 찼나")은 결제가 들어오는 순간 바뀌어 캐시된 답으로 팔면 초과 판매다. 인증 화면·Server Action·운영자 화면 뒤에서만. 쓰는 곳은 결제 완료 액션 하나.
    ⚠️ **같은 파일의 `getActiveExposure`는 예외로 `'use cache'`다** — 그건 "오늘 이 공고가 어떤 등급인가"라는 **표시용** 질문이고, 공개 목록과 같은 엔트리 수명·같은 `cacheTag("jobs")`를 쓴다(결제 액션의 `updateTag`가 함께 비운다). 판매와 표시를 한 파일에서 가른 이유는 그 머리말에.
...
- ✅ **읽기는 전부 실 DB다**(2026-08-22 전환 완료 · `src/mocks/` 삭제). 쓰기는 **수집 검수 판정 · 공개 공고 저장·마감 · 교회 인증 신청 접수 · 교회 인증 판정 · 교회의 공고 등록·수정·마감 · 교회 정보 저장 · 북마크**까지 왔다(2026-08-28). **클레임(가져오기)도 왔다**(2026-09-01 — `/jobs/new` 등록 전 후보 패널 + `claimJob`). **노출 결제 주문 저장도 왔다**(2026-09-03 — `completePromotion`: PortOne 조회 → 정원·겹침 재확인(경합이면 전액 취소) → `job_promotions` INSERT(멱등) → `updateTag("jobs")`). **원장 한 줄이 곧 노출이다** — `jobs`에 노출 칸이 없어 "적용" 단계가 따로 없다. 사진 업로드는 Phase 2로 미루고 화면도 걷어냈다(2026-08-29).
